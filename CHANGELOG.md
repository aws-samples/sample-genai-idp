...
- **Cross-account Bedrock invocation via STS AssumeRole** ([#305](https://github.com/aws-solutions-library-samples/accelerated-intelligent-document-processing-on-aws/issues/305)) — IDP processing Lambdas can now route all Amazon Bedrock data-plane and control-plane traffic through a centralized "hub" AWS account. Set the new optional CloudFormation parameter `BedrockHubRoleArn` (with optional `BedrockHubRoleExternalId` and `BedrockHubRoleSessionName`) to the ARN of an IAM role in a hub account; the stack conditionally adds `sts:AssumeRole` to each processing Lambda's execution role and injects the role configuration as environment variables. A new `idp_common.bedrock.session.get_bedrock_session()` factory wraps `botocore.credentials.DeferredRefreshableCredentials` so STS credentials are auto-refreshed before expiry — warm Lambdas don't break after the default 1-hour STS session. The `BedrockClient` data-plane and control-plane clients, the embeddings path, the Strands `BedrockModel` agentic-extraction path, and the multi-doc-discovery Embed/Cluster/Analyze/Save Lambdas all use this shared session. The change is fully additive: existing same-account `bedrock:*` IAM statements are kept in place so leaving the parameter empty preserves prior behavior. **Out of scope for v1**: BDA (Pattern-1) runtime calls and `idp_common/model_finetuning/` utilities. See [`docs/cross-account-bedrock.md`](docs/cross-account-bedrock.md) for the architecture, sample hub-role policies, resource-scoping caveats (application inference profiles and Guardrail IDs must be in the hub account), and the STS VPC endpoint requirement for private-network deployments.
