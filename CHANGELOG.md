...
- **In-app feedback & GitHub issue reporting.** Users can report bugs or request features on the project's public GitHub repository directly from the web UI, with deployment details pre-filled. Three entry points: a **Feedback** menu in the top navigation (Report a bug / Request a feature / View existing issues), a **Report this issue on GitHub** button in the [Error Analyzer](docs/error-analyzer.md) Troubleshoot modal footer (pre-fills the bug form with document context — object key, status, config version, execution ARN, job error — plus the agent's Markdown findings, alongside a **Copy full details** clipboard button for content the length-capped URL can't carry), and a **Report an issue** link in the side-nav Resources section plus a **Feedback & support** block in the Document List help panel. The bug/feature links target GitHub issue *forms* (`.github/ISSUE_TEMPLATE/*.yml`, converted from the prior Markdown templates) and pre-fill the AWS Region, Processing Mode, and an environment summary (Version, Build, Stack name) sourced from the deployment settings; field-level pre-fill activates once the forms are present on the repository's default branch. Nothing is submitted automatically — GitHub always shows the pre-filled form to review first, and the forms carry a prominent reminder to redact sensitive document data (issues on the public repo are visible to everyone). See [web-ui.md](docs/web-ui.md#feedback--issue-reporting).
