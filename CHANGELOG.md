...
- **Central input-shape validation on the UI API (restores AppSync schema validation).** The `POST /op/{field}` dispatcher now validates every request's `arguments` against a build-time spec generated from `schema.graphql` — rejecting unknown arguments, missing required (non-null) arguments, wrong scalar types, and out-of-set enum values with HTTP 400, before the request reaches a resolver. This restores the input-type gate AppSync enforced for free (and lost in the REST migration), closing a type-confusion / unexpected-argument defense-in-depth gap. The validator is stdlib-only (no runtime GraphQL dependency), conservative (type-only checks, shallow input-object validation), and fails open on its own internal errors so a validator bug can never take down the API. A drift guard keeps the spec in sync with the schema in CI. Threat model updated (AUTH.T12); see [AppSync→REST migration §4](docs/migration-appsync-to-rest.md).
