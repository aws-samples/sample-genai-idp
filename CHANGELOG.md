...
- **Test Studio: run-level graded packet metrics.** The V-measure / Rand-index / ordering scores computed per document by `compute_graded_packet_metrics` (added in the stickler integration cleanup — see below) landed in each doc's `evaluation/results.json` but never made it into the run-level dashboard, so a test run could not see how its packets scored on the partial-correctness axis the exact-match counters ignore. The test-execution-aggregation Lambda now reads `doc_split_metrics.{final_score, clustering_score, v_measure, rand_index, avg_ordering_score}` from each per-doc payload and emits a run-level bundle (`{mean, per_document, document_count}`) — the mean is a simple unweighted average across documents that reported each key, matching the `weighted_overall_scores` idiom (per-doc scores are already page-count-aware within a document). Docs that predate the field, and classification runs where `evaluate_packet` returned nothing, are absent from the map rather than zero-filled. Threaded through the resolver's cache write/read, the `TestRun.gradedPacketMetrics` GraphQL field, and the Test Studio UI's Additional Metrics panel (compact rows under the existing accuracy/split section — hidden entirely when no doc reported graded metrics). Existing cached runs re-aggregate once via the stale-cache guard: because that guard is a presence check, every run cached before this release trips it exactly once, so it now queues the recompute asynchronously and serves the already-cached metrics meanwhile (the graded panel simply stays hidden until the recompute lands). Previously that path fell through to the end of the resolver and returned nothing, which would have resolved `getTestRun` to `null` — "No test results found" in the UI, and silently dropped from `compareTestRuns` — for every historical run.
- **Test Set - ConfBench extension: on-demand deployment of the noise-augmented FCC invoice benchmark.** New optional [Feature Platform](docs/feature-platform.md) extension (`feature-platform/confbench-testset/`) that deploys the [amazon/ConfBench](https://huggingface.co/datasets/amazon/ConfBench) benchmark — the 75 verified FCC invoices of `realkie-fcc-verified`, each degraded by up to 21 Augraphy noise pipelines, 1,346 documents total — into Test Studio for confidence-calibration and OCR-robustness work. Ground truth is byte-identical to `realkie-fcc-verified`, so accuracy on any degraded variant is directly comparable to the clean baseline. Shipped as an extension rather than a fifth pre-deployed test set because the full dataset is **32.71 GB**, ~42x the combined size of the auto-deployed benchmark sets: deploying it unconditionally would charge every deployment for a specialist research dataset. Installing creates only the machinery (under a minute, downloads nothing); an admin then picks a size tier — clean baseline 0.02 GB / light noise 0.31 GB / representative spread 4.22 GB / full 32.71 GB — or hand-picks individual variants, with exact per-variant sizes and an approximate monthly storage cost shown before confirming. Sizes are very uneven (`original` 0.02 GB vs `custom15` 7.12 GB), so subsetting is a real lever rather than a token one. Ingest runs as a Step Functions job: plan the parquet row index once, `Map` over variants 4-at-a-time streaming CDN->S3 via bounded multipart, then finalize by counting what actually landed. Nothing waits on a CloudFormation response, so a long transfer can never fail a stack operation, and partial failures still register the test set with a per-variant failure report in S3. Note the dataset contains **21** distinct noise variants, not the "up to 18" its dataset card states — five are partial (covering 20-46 of 75 documents), which the picker labels explicitly since cross-variant accuracy comparisons over different document populations are not like-for-like. Reworked from the always-on main-stack deployer contributed in PR #583 by @sujimart, which carried the CloudFormation 1-hour custom-resource deadline as a stack-rollback risk. Admin-only install/ingest/delete. Each ingested test set records the extension's Invoice config version on itself, so Test Studio preselects it (see the `TestSet.configVersion` entry below). See [Test Set - ConfBench](docs/extensions/confbench-testset.md).

- **Test sets can declare which configuration version Test Studio preselects.** New optional `TestSet.configVersion` field: when a test-set record carries it, selecting that test set in **Run Test Set** preselects that configuration version. Test Studio previously inferred the configuration purely by name — it looked for a config version whose name *equals* the test set id, which is how the stack-managed benchmark sets (`fake-w2`, `docsplit`, `ocr-benchmark`, `realkie-fcc-verified`) get their pairing. That convention structurally cannot serve extension-deployed test sets, because the Feature Platform names every extension config preset `<featureId>-v<version>`, which can never equal a test set id — so those test sets always fell back to the active version and the admin had to re-pick the right configuration on every run. Resolution order is now: declared `configVersion`, then the id-equals-name convention, then the active version; a declared version that isn't in the dropdown (deleted, or outside the caller's config-version scope) falls through rather than leaving the field empty. Nothing changes for the managed benchmark sets, which don't set the field. The Test Set - ConfBench extension is the first consumer.
