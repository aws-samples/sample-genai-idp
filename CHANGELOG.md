...
- **Generate synthetic test sets from Test Studio (IDP Data Generator extension).** When the IDP Data Generator extension is installed, the **Test Studio → Test Sets** page gains a **Generate Synthetic Data** button that opens a modal to generate labeled synthetic documents — either from a plain-language document-type description or from an existing configuration version — as a background job that lands as a new test set. The generator ships as an installable Feature Platform extension (`feature-platform/idp-data-generator/`, AgentCore Runtime) that bundles `seed-data` + the accelerator's synthesis adapter; the same capability is what the Quick Start agent discovers and delegates to. _(Extension deploy path is staged/not yet deploy-verified end-to-end.)_
