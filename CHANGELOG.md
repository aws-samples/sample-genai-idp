...
- **AWS AppSync fully removed — replaced by API Gateway HTTP API + polling + Lambda response streaming (GovCloud / FedRAMP)** — The UI↔backend transport no longer uses AWS AppSync at all (it is unavailable in GovCloud, not FedRAMP-compliant, and being de-emphasized by AWS). There is **no toggle** — the HTTP API is the only transport. Specifically: (1) UI queries/mutations go to an **API Gateway HTTP API** (Cognito JWT authorizer) fronting a single dispatcher Lambda that reuses the existing resolver Lambdas — a shared adapter (`idp_common.api_adapter`) normalizes the HTTP API event to the resolver shape and restores the JWT authorizer's flattened `cognito:groups` claim to a list so RBAC is unchanged; DynamoDB-direct operations (discovery/agent jobs, `getDocument`, date queries) are served in-process by the dispatcher; (2) the backend writes the TrackingTable directly (`DocumentDynamoDBService`; the document-service factory is now DynamoDB-only) and the **5 former status subscriptions are polling** in the UI (reusing the dedup/merge logic), pausing while the browser tab is hidden; (3) the **two chat flows stream via a direct Lambda Function URL** (`AuthType=AWS_IAM` + browser SigV4 with Cognito Identity Pool creds, `InvokeMode=RESPONSE_STREAM` via the AWS Lambda Web Adapter), working identically under CloudFront or ALB hosting; (4) **all `AWS::AppSync::*` resources are deleted** (GraphQL API, ~150 data sources/resolvers/schema across the main, nested, pattern, and feature-platform stacks), and the `idp_common.appsync` module is removed. The **Feature Platform** is migrated too: its 6 UI fields route through the HTTP API dispatcher and its 6 install hooks (register/unregister feature & hooks, apply/remove config preset) are invoked **directly** by feature stacks via re-exported Lambda ARNs instead of AppSync mutations. Validated end-to-end on a fresh stack (zero AppSync resources; document processed to COMPLETED; HTTP API authenticated reads + DDB-direct + 401-on-no-auth confirmed).
