...
- **Optional AppSync-free transport (API Gateway HTTP API + polling + Lambda response streaming) for GovCloud / FedRAMP** — A new `ApiTransport` stack parameter (`appsync` default, or `httpapi`) lets the solution run with **no AWS AppSync dependency**, which is required because AppSync is unavailable in GovCloud, is not FedRAMP-compliant, and is being de-emphasized by AWS. When `ApiTransport=httpapi`: (1) UI queries/mutations go to an **API Gateway HTTP API** (Cognito JWT authorizer) fronting a single dispatcher Lambda that reuses the existing resolver Lambdas — a shared adapter (`idp_common.api_adapter`) normalizes the HTTP API event to the AppSync resolver shape and restores the JWT authorizer's flattened `cognito:groups` claim to a list so RBAC is unchanged; DynamoDB-direct resolvers (discovery/agent jobs, `getDocument`) are ported to the dispatcher; (2) the backend writes the TrackingTable directly (`DOCUMENT_TRACKING_MODE=dynamodb`) and the **5 status subscriptions become polling** in the UI (reusing the existing dedup/merge logic), pausing while the browser tab is hidden; (3) the **two chat flows stream via a direct Lambda Function URL** (`AuthType=AWS_IAM` + browser SigV4 with Cognito Identity Pool creds, `InvokeMode=RESPONSE_STREAM` via the AWS Lambda Web Adapter), so token-by-token streaming works identically under CloudFront or ALB hosting; (4) all `AWS::AppSync::*` resources are gated on a `UseAppSync` condition so they are **not created at all** under `httpapi`. The default (`appsync`) path is byte-for-byte unchanged. The UI selects transport at build time via `VITE_API_TRANSPORT`. Known limitation: the optional Feature Platform (`EnableFeaturePlatform`, default off) still registers features via an AppSync mutation and is a separate follow-up.
