...
- **The Configuration editor showed "Configuration saved successfully" and "You have unsaved changes" at the same time.** The green success banner was state that only cleared on an explicit dismiss, while the blue unsaved-changes banner is derived live from the form contents — so the first edit after a save left both on screen, one saying the work was persisted and the other saying it was not. The success banner is now dismissed by the edit itself, from every entry point that can dirty the version (form view, JSON and YAML editors, version description, Document Schema and Policy Schema tabs). Dismissal is driven by the edit handlers rather than by the derived `hasUnsavedChanges` flag, because that flag is transiently true after a successful save while the form re-syncs from the newly persisted config — gating on it would have hidden the confirmation the user just earned. The "Configuration imported successfully" banner deliberately still co-exists with the unsaved-changes banner, since there the import *is* the pending change. (#616)
