...
- **Agentic extraction: tolerate string `minItems` in schema analysis** — `_analyze_schema_for_table_requirements` coerces `minItems` to an int before the `> 50` large-table comparison. The Configuration table stores numeric schema fields as strings, so a class schema with an array `minItems` (the recommended way to flag a large table for completeness) previously crashed the shard-planning step with `TypeError: '>' not supported between instances of 'str' and 'int'`.
