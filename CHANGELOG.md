...
- **`clip-under-threshold` and `aggregate` per-field pass-throughs.** `SticklerConfigMapper` now translates `x-aws-idp-evaluation-clip-under-threshold` and `x-aws-idp-evaluation-aggregate` into the corresponding `x-aws-stickler-*` keys, exposing Stickler's `JsonSchemaFieldConverter` knobs at the neutral IDP-extension surface.
- **Field-count-weighted document-level rollup.** `EvaluationService.evaluate_document` now weights each section's `weighted_overall_score` by its field count (from Stickler's aggregate counts) rather than taking an unweighted mean over sections, so a 20-field section outweighs a 2-field one. Matches the aggregation Lambda's Stickler-native bulk rollup. Falls back to the unweighted mean when no section reports counts (typically only sections that failed evaluation).
- **Graded packet metrics on `DocSplitMetrics`.** Every document-split evaluation now also reports five graded metrics from Stickler's `stickler.doc_split.packet_evaluation_metrics.evaluate_packet`: `final_score`, `clustering_score`, `v_measure`, `rand_index`, `avg_ordering_score` (all in `[0.0, 1.0]`). These complement the existing exact-match counters — a 9/10-page section scores near 1.0 here but 0/1 under `split_accuracy_without_order`. Populated by `service._compute_graded_packet_metrics`; failure is soft (metrics stay `None` on error). Flows through `evaluation_result.doc_split_metrics.to_dict()` into per-doc `results.json`.
- **Evaluation golden-fixture regression suite.** New unit tests at `lib/idp_common_pkg/tests/unit/evaluation/test_golden_fixture_stickler.py` run the real (unmocked) `stickler-eval==0.5.0` against three checked-in fixtures — a flat W2-style doc (EXACT/FUZZY/NUMERIC_EXACT/DATE), a nested invoice with a Hungarian-matched array, and a doc-split scenario — and byte-compare each result (including the full `stickler_comparison_result` blob consumed downstream by the aggregation Lambda) against a golden JSON. Regenerate deliberately via `python scripts/regenerate_evaluation_goldens.py` and inspect the diff — every future stickler-facing change is now gated on this suite.

### Removed

- **`ConfidenceMetricsCalculator` class** (`idp_common.evaluation.confidence_integration`). Had no production caller — only doc references — and was subsumed by the aggregation Lambda's use of Stickler's `BulkStructuredModelEvaluator` with confidence metrics installed as bulk-level accumulators. The still-used `get_average_confidence_from_metrics` helper remains in the module.
- **Legacy `SticklerConfigMapper` fields-config path** (~200 lines: `map_field_config`, `_map_type`, `_resolve_ref`, `JSON_SCHEMA_TO_STICKLER_TYPE`). Production only uses `build_stickler_model_config` → `_translate_extensions_in_schema` → Stickler's own `JsonSchemaFieldConverter`; NUMERIC_EXACT tolerance handling (the one behavior the legacy path did better) was lifted into the production path in Phase 1.
- **`verify_stickler.py`** (195 lines, `lib/idp_common_pkg/verify_stickler.py`). It smoke-tested `model_from_json` — a path production never exercises — and was wired into no Makefile/CI target. The golden-fixture suite added in this release covers the actual production path (`JsonSchemaFieldConverter` → `create_model` → `compare_with`) end-to-end.
- **`munkres` dependency.** Nothing in `idp_common` imported it; Hungarian matching lives inside Stickler. Dropped from `evaluation`, `dev`, `test`, and `all` extras in both `pyproject.toml` and `setup.py`.
- **`idp_common.evaluation.metrics` module (`metrics.py`, 145 lines).** All of `precision`, `recall`, `f1_score`, `accuracy`, `false_alarm_rate`, `false_discovery_rate`, `calculate_metrics` were re-implementations of Stickler's `DerivedMetricsCalculator` + first-class `fa`/`fd` cells. Document-level derived metrics are now computed inline in `service._compute_derived_metrics` from the summed Stickler counts. The module-level `calculate_metrics` re-export from `idp_common.evaluation.__init__` is gone; no consumer outside the package used it.
- **Non-null default injection for missing required fields.** `EvaluationService._coerce_data_to_schema` no longer runs its second pass to fill absent required fields with type-appropriate defaults (`""`, `0`, `0.0`, `False`, `[]`, `{}`) — nor does `_get_default_for_type` (also deleted). The path was already dead (the mapper clears `required` and the genson auto-schema strips it) but dangerous: injecting `0` for a genuinely-absent numeric would flip a false negative into a value mismatch (`fd`), inflating error counts.
- **Local `DocSplitClassificationMetrics` fork (~650 lines).** `idp_common/evaluation/doc_split_classification_metrics.py` was AST-identical to the upstream implementation shipped in `stickler.doc_split` since 0.2.0 (upstream docstring notes it was "Ported from the GenAI IDP Accelerator"). Callers now import `stickler.doc_split.doc_split_classification_metrics.DocSplitClassificationMetrics` directly — the concrete module, not the package, so pandas/scipy/sklearn are not pulled into Lambda cold start via `stickler.doc_split.__init__`. A small S3-loading adapter (`EvaluationService._load_sections_for_doc_split`) still bridges IDP's `Section` objects to the plain-dict shape the upstream calculator accepts. The dead `generate_markdown_report` reference in the fork (zero callers repo-wide) is gone with it; the live markdown renderer stays in `DocumentEvaluationResult.to_markdown`. Corresponding "Usage / Visual Reporting / Example: Complete Workflow" sections in `lib/idp_common_pkg/idp_common/evaluation/README.md` were rewritten to point at the service-owned rendering path.

### Changed

- **Evaluation package restructured behind a single Stickler import boundary (§6 reorg).** New subpackage `lib/idp_common_pkg/idp_common/evaluation/stickler_backend/` is now the ONLY code path allowed to `import stickler`. Contains `mapper.py` (was `stickler_mapper.py`), `comparators.py` (was `llm_comparator.py`; also owns `register_idp_comparators`), `model_factory.py` (was `EvaluationService._get_stickler_model` + `_make_model_fields_nullable` + `_clean_null_descriptions`), `results.py` (was `EvaluationService._transform_stickler_result` + `_annotate_nested_comparison_methods` + `_resolve_leaf_schema`; encodes R3 as a pure Stickler→IDP converter), and `doc_split.py` (was `EvaluationService._load_sections_for_doc_split` + `service._compute_graded_packet_metrics`). New `contract.py` at the package root formalizes the cross-Lambda `results.json` API — exports the S3 key template, the `compare_with` flag set, and a `STICKLER_RESULT_VERSION` stamp — so a shape change fails loudly at read time in the aggregation Lambda instead of as silent-wrong dashboard numbers. Historical top-level modules (`stickler_mapper.py`, `llm_comparator.py`) survive as thin re-export shims so external callers using the old import paths keep working. `service.py` demoted to orchestration only.
- **R7 landed: sklearn-based confidence post-pass replaced by a Stickler-native accumulator subclass.** The aggregation Lambda's `_enhance_confidence_metrics_with_patterns` (170 lines of regex path-collapse plus `sklearn.metrics.roc_auc_score` / `brier_score_loss`) was deleted. Replaced with `_IndexCollapsingConfidenceAccumulator` — a `ConfidenceAccumulator`-shaped duck-type that collapses `LineItems[N].Rate → LineItems.Rate` inside `accumulate()` and delegates AUROC / ECE / Brier to Stickler's built-in `ConfidenceCalculator`. Wired as an `accumulators=[...]` on `BulkStructuredModelEvaluator` (the same API already used for ECARB@30), so all confidence metrics flow through Stickler's supported extension point instead of a post-pass. Output shape now matches Stickler's native `confidence_metrics` schema.
- **`stickler_version.STICKLER_VERSION` now derived at import time from `importlib.metadata.version("stickler-eval")`** (previously a hardcoded literal that had already drifted from what the resolver installed). The hand-maintained `STICKLER_FEATURES` list was removed. A unit test asserts `STICKLER_VERSION == importlib.metadata.version("stickler-eval")` so both env-drift and pin-vs-constant drift fail loudly.
- **Numpy pin relaxed to `>=1.26,<3`** in the `evaluation` and `test` extras to match stickler-eval's declared range. Exact-pinning to `1.26.4` invited resolver conflicts on upgrade.
- **Evaluation docs sweep across both tiers** (`docs/evaluation.md`, `docs/evaluation-enhanced-reporting.md`, `docs/test-studio.md`, and `lib/idp_common_pkg/idp_common/evaluation/README.md`): dropped stale "v0.4.0 / v0.1.4 / feature branch / PR #20 / sticker-eval typo" references; version now comes from the runtime constant. `notebooks/examples/step7_evaluation.ipynb` pins its `%pip install stickler-eval` to match the top-level pin. The `EvaluationMethod` enum in `idp_common/evaluation/models.py` gained a `LEVENSHTEIN` member (the string was already permitted by `schema_constants`). The evaluation-methods appendix rendered into the markdown report now uses the neutral IDP extension key (`x-aws-idp-evaluation-weight`) and documents the DATE method.
- **Evaluation verdicts and counts now sourced from Stickler.** `EvaluationService._transform_stickler_result` no longer re-derives per-field `matched` / `tp/fp/fn/tn/fp1/fp2` from IDP's private threshold table. Verdicts come from `stickler_result["confusion_matrix"]["fields"][name]["overall"]` (tp+tn>0 → matched); section-level counts come from `stickler_result["confusion_matrix"]["aggregate"]`; `precision/recall/f1_score/accuracy` come from `derived.cm_*`; `false_alarm_rate` and `false_discovery_rate` are now computed from Stickler's `fa` and `fd` cells (previously IDP's `fp1`/`fp2` re-count) — same formula as the aggregation Lambda, so per-doc and run-level dashboards now report identical rates. Users may see FAR/FDR shifts on any field whose IDP-derived counts disagreed with Stickler's (e.g. floating-point-epsilon boundary matches; unconfigured fields now honoring Stickler's `clip_under_threshold` default). `SectionEvaluationResult.metrics` carries the raw counts under a `_stickler_counts` sub-dict for downstream aggregation.
