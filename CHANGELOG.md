...
- **Mandatory AppSec API security test cases in the integration harness.** The live API test (`make api-test`, CodeBuild Step 12) now covers the AppSec "Minimum Mandatory Security Focused Test Cases for APIs" checklist beyond the existing role permission matrix: IDOR/BOLA (a second user cannot reach another user's chat session), token expiry and post-logout revocation (the stateless-JWT reuse window is surfaced as a documented gap), deleted-resource inaccessibility, input validation (tolerant now, strict via `IDP_SECTEST_STRICT_INPUT`), and TLS (TLS 1.0/1.1 and cleartext HTTP are refused). Implemented in `scripts/api_security_cases.py` with unit tests; threat model updated (AUTH.T09–T11). See [api-rbac-test skill](.claude/skills/api-rbac-test.md) and [CI test coverage](scripts/sdlc/docs/CI_TEST_COVERAGE.md).
...
- **CI: the expensive deploy jobs now skip doc-only commits.** `integration_tests` (~70 min real-AWS deploy) and `deployment_validation` only auto-run when a commit/MR touches deploy-affecting paths (`template.yaml`, `patterns/`, `nested/`, `src/`, `lib/`, `config_library/`, `feature-platform/`, `iam-roles/`, `scripts/`, `publish.py`, `requirements*.txt`, `.gitlab-ci.yml`). A commit that changes only `VERSION`, `CHANGELOG.md`, `**/*.md`, `docs/`, `images/`, etc. no longer triggers the deploy. The allowlist is intentionally generous (a false "run" only wastes CI minutes; a false "skip" could merge a broken deploy). `fast_checks` (lint/SRT) still runs on every commit; a Draft-MR manual deploy button is always available regardless of the filter.
...
- **Synthetic document generator now uses the published SEED package (`seed-data`).** The Quick Start / `idp-cli bootstrap` document generator integrates the open-source [SEED](https://github.com/awslabs/synthetically_engineered_evaluation_data) package (`pip install seed-data`, or the `idp_common[synthesis-generator]` extra) via its typed `Generator` API. It stays an optional, lazily-imported add-on: schema authoring, catalog matching, and config/test-set creation all work without it, and the generator install is a separate extra so the base synthesis capability stays lightweight.

- **Generate synthetic test sets from Test Studio (Test Set Generator extension).** When the Test Set Generator extension is installed (feature id `idp-data-generator`), the **Test Studio → Test Sets** page gains a **Generate Synthetic Data** button that opens a modal to generate labeled synthetic documents — either from a plain-language document-type description or from an existing configuration version — as a background job that lands as a new test set. The modal adds an optional **scenario** theme (with a Bedrock-backed **Suggest** helper), a **quality** setting (faster vs. higher quality), and a live **cost/time estimate**; generated documents can be previewed in-place from the Test Sets list without running a test execution, and a **Generate test set** button on the **View/Edit Configuration → Document Schema** tab deep-links into the pre-filled modal. The generator ships as an installable Feature Platform extension (`feature-platform/idp-data-generator/`, AgentCore Runtime) that bundles `seed-data` + the accelerator's synthesis adapter; the same capability is what the Quick Start agent discovers and delegates to. In-flight jobs are surfaced on the Test Sets page even when started from Quick Start, and a runtime watchdog fails a job (rather than leaving it stuck IN_PROGRESS) if a run wedges past the AgentCore session ceiling (~8h; env-tunable). _(Extension deploy path is staged/not yet deploy-verified end-to-end.)_
