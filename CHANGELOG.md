...
- **Policy Discovery had four ways to quietly do less than asked.** Each ended in the same place: a job reporting success while rule validation produced nothing, or silently less than the policy required.
  - **A ruleset truncated at the model's output-token limit was persisted as if complete (#603).** Extraction validated *structure*, not *completeness*, so a response cut off after rule 60 of 140 that happened to close its braces — or that the tolerant JSON recovery salvaged to a valid prefix — passed every check. The job logged "Successfully extracted 1 rule classes", the UI reported success, and 80 rules were absent from the compliance validation set with nothing anywhere saying so. Both the traditional and agentic paths now check the model's stop reason (`max_tokens`, plus the OpenAI Responses adapter's `max_output_tokens`/`incomplete`), retry, and **fail the job** rather than saving a knowingly-partial ruleset — for a compliance feature a failed job beats a validation set that is quietly 43% of the policy. The failure message names both remedies (split the document, or raise `discovery.rules.max_tokens`) and reaches the UI.
