...
- **The "Extensions" side nav now lists installed features only.** Previously the nav also listed every catalog feature — so the two bundled samples (*Sample: Document Status (feature add-on)* and *Sample: Health Insurance Review*) appeared with Install/Subscribe badges even on stacks where they were never deployed. Catalog-only (published but not installed) features are now discovered via a new **Browse catalog** nav link, which opens a catalog browser page (`/features`) listing every extension — installed and available — with status badges and links to each feature's install/update page. Once a feature is installed it gets its own nav entry (with the existing "Update" badge when a newer catalog version ships). See [docs/feature-platform.md](docs/feature-platform.md).
