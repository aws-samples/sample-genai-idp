...
- **A `postprocessing` pipeline hook, symmetrical with `preprocessing`, as the workflow's final optional step — and it can modify the document.** The accelerator had a `preprocessing` hook point running *inside* the state machine (config-version scoped, admin-editable, able to mutate or halt the document) but its counterpart, the `PostProcessingLambdaHookFunctionArn` stack parameter, was a fundamentally different mechanism: an EventBridge rule firing **after** the execution, invoking a customer Lambda asynchronously with a document *snapshot*. It could not change the document, could not gate it, could not be swapped by activating a config version, and had no `args`, no `enabled` toggle, and no `onError` policy. There is now a real `postprocessing` extension point on the shared workflow tail — after evaluation, before the terminal state, so it fires in both BDA and pipeline modes — that is the mirror image of `preprocessing`: the same standalone top-level config section holding a single flat hook (`enabled` / `arn` / `onError` / `args` / `featureId` / `allowDocumentUpdate`), the same dispatcher, the same `updatedDocument` mutation contract and guardrails, and the same `idp_common.hooks` helpers. It appears in the View/Edit Configuration UI as **Postprocessing**, directly below Evaluation. Because it sits at the end of the tail, a mutation there is the last word on the document: `WorkflowComplete`'s output is what the workflow tracker persists, so changes reach the tracking row, the reporting/Athena rows, and the UI. Three deliberate asymmetries with `preprocessing`, all following from the document already being fully processed by the time it runs: `onError` defaults to `continue` and is documented against `fail` (which would mark an otherwise-successful document FAILED for a delivery-integration error), whereas a failed `preprocessing` hook *must* be terminal; `halt` is not actionable (there is nothing downstream to skip), so the dispatcher logs it and reports `haltIgnored: true` rather than appearing to act on it; and a mutation cannot set a terminal `status`, since a successful execution is forced to `COMPLETED`. The hook is **not** skipped while a HITL review is pending — it receives the document's `hitl_status` / `hitl_triggered` / `hitl_sections_pending` fields and decides for itself, which means it fires once on the interim completion and again after review, so hooks here must be idempotent (note those fields are omitted when falsy, so absent means "no HITL"). Both `preprocessing` and `postprocessing` are now registrable by Feature Platform features via `registerFeatureHooks`; `preprocessing` was listed in the feature manifest schema but rejected by the resolver, which is why the PII Anonymization extension had to bake its ARN into a config preset instead. Each flat point holds exactly one hook, so registration that would overwrite another feature's hook is refused rather than silently disabling it, and a feature's preset `args` survive registration. Inert by default. See [docs/feature-platform.md](docs/feature-platform.md#pipeline-hooks) and the [developer guide](docs/feature-platform-developer-guide.md).

- **`allowDocumentUpdate` is now editable in the Configuration UI.** The per-hook kill-switch that pins a hook to observe-only was enforced by the dispatcher and documented for admins, but was absent from the config schema — so the UI could not set it. Added to the `preprocessing` and `postprocessing` sections.
...
- **The Method column and the below-threshold reason disagreed on the threshold value for fields with a bare method annotation.** For fields declared `x-aws-idp-evaluation-method: FUZZY` (or `SEMANTIC` / `LEVENSHTEIN`) **without** a paired `x-aws-idp-evaluation-threshold`, the per-field display in the Document detail's Section Visual Editor read `Method: Fuzzy (threshold: 0.70)` next to `Eval Score: 0.0% - below threshold (0.000 < 0.5)` — same field, same run, two different numbers. Two fallbacks disagreed: Stickler's JSON-schema converter constructs `ComparableField(threshold=0.5)` in that case (a hardcoded default at `stickler/structured_object_evaluator/models/json_schema_field_converter.py:159`, mirrored by `ConfigurationHelper.get_comparison_info`'s `comparison_config.get("threshold", 0.5)`), so scoring used 0.5 and the reason string embedded it; `_format_evaluation_method` in `idp_common.evaluation.service` — when its `field_specific_threshold` came in as `None` because the schema had no `x-aws-stickler-threshold` — fell back to a hardcoded per-method table `_THRESHOLD_BASED_METHODS = {"Fuzzy": 0.7, ...}` and rendered `0.70`. The fix reads the applied threshold from Stickler's model rather than the schema: `ComparableField` unconditionally stashes the resolved threshold on `field_info.json_schema_extra._threshold` at model build time — the same value `ConfigurationHelper` returns at compare time — so `stickler_backend/results.transform_stickler_result` and `annotate_nested_comparison_methods` now thread `root_model_cls=type(expected_instance)` through and read that value for the Method display string. The dataclass's `evaluation_threshold` field is unchanged (still populated from the user's explicit configuration / NUMERIC_EXACT tolerance), so downstream consumers of the *configured* value are not affected — only the *displayed* value moves. `_THRESHOLD_BASED_METHODS` is deleted, replaced by a bare `frozenset` of method names that render a threshold suffix; when no threshold reaches `_format_evaluation_method` at all (rare edge — auto-generated section that didn't build a model), the suffix is omitted rather than guessing. The bug rendered on both the **Section Visual Editor** (per-field via `FormFieldRenderer.tsx`) and the **Evaluation Report** (Method column via the HTML renderer in `evaluation/models.py`), both reached from the Document detail; Test Studio doesn't display this string. Verified live: 32 bare-FUZZY leaves in a `healthcare-multisection-package` run (`PatientInformation.Patient-Name`, `HospitalName`, `MedicationName`, `InsuranceDetails.Member-ID`, and so on) all render `Fuzzy (threshold: 0.50)` after the fix; pre-fix they printed `0.70`.

- **A completed test run reported `EVALUATING` forever — alongside `10/10 processed` and `0 evaluating` — whenever the test set name contained a space or parenthesis.** The run had genuinely finished (every document `COMPLETED`/`EvaluationStatus=COMPLETED`, and Stickler had already computed `overall_accuracy=0.7232` for it), but its aggregate metrics were never cached, and `get_test_run_status` deliberately reports `EVALUATING` while a terminal run has no `testRunResult`. That override was the badge; `evaluatingFiles` was correctly `0`, which is what made the two halves of the display look self-contradictory. The metrics were destroyed by a **context confusion in the SQL-injection guard**: `test_run_id` is derived from the user-chosen test set name and is interpolated into a string *literal* (`WHERE document_id LIKE '{test_run_id}%'`), but it was checked against the identifier allow-list `^[a-zA-Z0-9_\-./]+$` — so a legitimate name like `ConfBench (light noise)` raised `test_run_id contains invalid characters`. The correct defense for a literal is escaping, not an allow-list, so `test_run_id` now goes through a `_sql_like_prefix()` helper that doubles `'` (the complete escape for a Trino literal, where backslash is *not* an escape character) and neutralises the `%`/`_` wildcards via a paired `ESCAPE` clause — the latter fixing a second latent bug where a name containing `_` silently widened the prefix match to other runs. The database name is a genuine identifier and keeps the strict allow-list. Two failure-containment bugs are fixed alongside, either of which alone would have prevented the permanent stall: the Athena calls for split-classification metrics and cost are only *supplements* to metrics Stickler has already computed, yet an exception from them propagated out through a broad `except` that discarded the good Stickler results (and logged the actively misleading `Test execution aggregation Lambda failed`, when the aggregation Lambda had succeeded) and on into `handle_cache_update_request`'s bare `except`, which swallowed it and let SQS delete the message — they are now best-effort and degrade to their documented "no data" shapes; and **nothing re-enqueued the aggregation**, because the enqueue in `get_test_run_status` fires only on a status *transition* and the stale-cache re-enqueue fires only when `testRunResult` already exists, so a run whose single aggregation attempt failed stayed metric-less permanently. Both `get_test_run_status` and `get_test_results` now re-enqueue when a terminal run has no metrics, throttled by an atomic conditional write on `CacheUpdateQueuedAt` — which also collapses the redundant duplicate aggregations observed during the incident, where three concurrent readers each fired their own. (#619)

- **`idp-cli status` and `run-inference --monitor` crashed for any document in one of four real processing states.** The SDK's `DocumentState` enum validates the `ObjectStatus` read straight out of the tracking table, and it had drifted behind `idp_common.models.Status` — so a document in a state the enum did not know about aborted monitoring with `1 validation error for DocumentStatus / Input should be 'QUEUED', ...` instead of reporting progress. Two of the four missing values are on entirely ordinary paths: **`PREPROCESSING`**, which the pipeline-hooks dispatcher sets for *every* document whenever a preprocessing hook is registered (so every [PII Anonymization](docs/extensions/pii-anonymizer.md) user hit this on every document), and **`RULE_VALIDATION_POLICY_CLASSIFICATION`**, set during rule validation's policy-classification phase. The other two are `REDACTED_SUPERSEDED` (the redact-and-stop terminal state) and `PENDING_UPLOAD` (Jobs API). Compounding it, `--monitor` swallows the error and still exits 0, so a batch looked like it had been waited for when it had not. All four are now present, and a new test asserts the enum is a **superset** of the runtime `Status` — as a set relationship, so a status added to the runtime in future fails offline in CI rather than in a user's CLI. Two related monitor bugs fixed alongside: `REDACTED_SUPERSEDED` is now treated as terminal (it never reaches `COMPLETED`, so monitoring previously waited for it until timeout) and counted as done rather than failed; and the progress display's "running" bucket was an explicit list that omitted `OCR`, `PREPROCESSING`, `POSTPROCESSING` and `RULE_VALIDATION_POLICY_CLASSIFICATION`, so documents in those states were shown as **Queued** — the buckets are now derived, so an unrecognised state defaults to running instead of silently misreporting.
...
- **The Configuration editor raised two identical "Configuration saved successfully" banners, and neither went away.** A save produced both an app-wide green toast at the top of the page and a second, near-identically worded inline Alert above the tabs — so the confirmation was duplicated, and both were still on screen after the user's next edit raised the blue "You have unsaved changes" banner, one saying the work was persisted and the other saying it was not. The inline Alert is removed: the toast is the one with a reason to exist (it stays visible when the user is scrolled deep into a long config form, which the inline banner above the tabs is not). **The toast now actually auto-dismisses**, which is a fix in `useNotifications` rather than in the config page — it always set a 5s dismissal timer, but also reset `successMessage` as it fired, and since that value is in the effect's dependency array the immediate re-run's cleanup cleared the timeout it had just set. Every success toast in the app therefore lingered until dismissed by hand, contrary to the stated intent of the code; pending timers now live in a ref cleared on unmount. One banner per save, gone in five seconds. Also: a save with nothing to persist reported "Configuration saved successfully" and now says "No changes to save."; the "Configuration imported successfully" Alert is unchanged, having no toast duplicate. (#616, #617)

- **`publish` could not build the ConfBench test-set extension from source.** `sam build` failed with `PythonPipBuilder:ResolveDependencies - {pyarrow==23.0.1(wheel)}`, blocking any publish with the extension enabled in `extensions-oss.yaml`. SAM's pip builder requests exactly one platform tag — `--platform manylinux2014_aarch64` — and pyarrow's last aarch64 wheel under that tag is 20.0.0; 21.0.0 onward ship `manylinux_2_28_aarch64` only. The failure therefore needed all of: an arm64 target, an x86_64 build host, pyarrow >= 21 and no container build, which is why this feature's arm64 functions broke while the x86_64 main-stack Lambdas on the identical pin did not. Downgrading was not available: pyarrow <= 23.0.0 is affected by CVE-2026-25087 (use-after-free, HIGH), fixed in the 23.0.1 pinned repo-wide. pyarrow now installs into the extension's makefile-built `SharedLayer` with the modern tag named explicitly — correct for the runtime, since python3.12 Lambda is AL2023 (glibc 2.34) — and all four ingest functions already mount that layer, so they now share one copy instead of embedding one each.

- **The optional post-processing Lambda hook silently never fired for large documents.** The `PostProcessingDecompressor` inflates the compressed document and then hands it on with `InvocationType='Event'`. An **asynchronous** Lambda invoke caps its payload at **1 MB** — the 6 MB limit applies only to synchronous invokes — so any document whose decompressed JSON exceeded that raised `RequestEntityTooLargeException`, which the handler re-raised into 3 EventBridge retries and then the DLQ. Reproduced live before fixing: the pre-fix code fails on a 1,441,631-byte document with `RequestEntityTooLargeException ... (limit 1048576 bytes)`, where the fixed code delivers the compressed reference (417 bytes) successfully. The hook therefore failed for exactly the big multi-section packets most likely to matter, and the decompression added for *convenience* is what broke it: the compressed wrapper it replaced was always small. The decompressor now measures the payload and, when it will not fit, invokes the customer Lambda with the **original compressed event** instead of failing — so the hook still fires, with a warning naming the size and `sentCompressedFallback: true` in the response. Documents under the limit are unaffected and still arrive decompressed. Documented, since a hook handling arbitrarily large documents must now check `document.compressed`.

- **The post-processing hook fired for PII-redacted originals it should never have seen.** When a `preprocessing` hook halts a document after spawning a redacted copy (the PII Anonymization extension's "redact copy and stop" mode), that execution still ends as `SUCCEEDED`, so the EventBridge rule matched and the customer's post-processor was invoked with the **un-redacted original** — precisely what the redaction existed to prevent — for a document the workflow tracker was about to delete. The status lives inside `detail.output`, which an EventBridge pattern cannot filter on, so the decompressor now checks it and skips the invocation for `REDACTED_SUPERSEDED`. The redacted copy is processed as its own document and fires the hook normally.
...
- **The settings cache's post-failure retry window is now an explicit deadline, instead of a timestamp back-dated past the monotonic clock's origin.** When an SSM refresh failed with an empty cache, `SettingsCache._refresh()` set `_cache_time = time.monotonic() - ttl + 30` to make the cache *look* nearly-expired so the next read would retry in ~30s. `time.monotonic()` on Linux counts from boot, so that subtraction can go negative on a young container and `_cache_time` stops meaning "a point in the past on the monotonic clock" — it becomes a value whose interpretation depends on the clock's origin. The retry *interval* itself survived this (the arithmetic is relative, so the next attempt still landed ~30s later regardless of origin), which is why it was never observed in Lambda; what did not survive was any **absolute** reading of `_cache_time`, and the unit test took exactly such a reading. Replaced with a forward-looking `_retry_after` monotonic deadline, so no stored value depends on the origin and the two windows are stated rather than encoded as a fake age: a full TTL when stale data is available (don't hammer SSM), a short 30s window when the cache is empty (don't run with no settings at all). Two latent bugs fall out of the rewrite: the empty-cache retry window is now **capped at the TTL**, where previously a cache configured with a TTL under 30s got a retry window *longer* than its own TTL; and `invalidate()` now clears a pending retry deadline, so "refresh now" is honoured mid-deferral rather than being suppressed by it. (#609)
...
- **The shipped `rule-validation` config preset evaluated 2 of its 14 rules.** The preset defines 7 NCCI policy classes but carried a `Document Name Regex` on only the **first** one, and rule validation evaluates only the classes whose regex matches — so the other 6 classes' 12 rules could never fire. Running the preset against its own bundled sample (`samples/rule-validation/medicare_respiratory_pa_packet.pdf`) matched 1 of 7 classes. The regex was also underscore-only (`prior_auth|pa_packet`), so the hyphenated filenames real intake systems produce (`Prior-Auth-123.pdf`) matched nothing at all. All 7 classes now carry the broadened `prior[_ -]?auth|pa[_ -]?packet` pattern; the sample now evaluates 14 of 14 rules, and an unrelated document still matches nothing. This is the `config_library` twin of the health-insurance feature preset fixed in #570 — that fix was applied to the feature's copy and this one was left behind. A new gate in `config_library/test_config_library.py` now fails on **any** preset with multiple policy classes where a class has neither regex, so a class can no longer ship unreachable.
...
- **Policy Discovery had four ways to quietly do less than asked.** Each ended in the same place: a job reporting success while rule validation produced nothing, or silently less than the policy required.
  - **A ruleset truncated at the model's output-token limit was persisted as if complete (#603).** Extraction validated *structure*, not *completeness*, so a response cut off after rule 60 of 140 that happened to close its braces — or that the tolerant JSON recovery salvaged to a valid prefix — passed every check. The job logged "Successfully extracted 1 rule classes", the UI reported success, and 80 rules were absent from the compliance validation set with nothing anywhere saying so. The **traditional** path now checks the model's stop reason (`max_tokens`, plus the OpenAI Responses adapter's `max_output_tokens`/`incomplete`), retries, and **fails the job** rather than saving a knowingly-partial ruleset — for a compliance feature a failed job beats a validation set that is quietly 43% of the policy. The **agentic** path was never silent here (Strands fails hard with `MaxTokensReachedException` before returning a result, and deliberately lets it bubble up), but it surfaced as an opaque "Agent has reached an unrecoverable state" with no indication of what to do; it is now translated into the same actionable error, so both paths name the same remedies (split the document, or raise `discovery.rules.max_tokens`). Note that the agentic envelope check is a deliberate belt-and-braces fallback rather than the primary guard: `structured_output()` synthesizes its response envelope without a `stopReason`, so it cannot fire today — it exists so that neither a Strands behavior change nor an envelope change can silently reopen the hole.
  - **The documented output shape was not the shape that works (#600).** `discovery_rules_from_document_local` returned raw LLM output keyed on `x-aws-idp-rule-type`, while the S3 path reshaped it to `policy_classes` / `x-aws-idp-policy-type` — the only key the runtime and the Policy Schema UI read. So the two entry points documented side by side in the same notebook produced silently incompatible results, and a notebook user following step 6 saved rules under a config key nothing reads. Compounding it, `rule_classes` (renamed to `policy_classes` in v0.5.9) was **deleted without a warning** when both keys were present, and being a known-deprecated key it did not trip the unknown-field warning either. The reshape is now a shared `to_policy_class()` used by both paths, so they cannot drift; the discard now logs a WARNING naming what was dropped; the module docstrings and the tutorial notebook say `policy_classes`; and the notebook's compatibility check now asserts the key the runtime actually requires (asserting the old one passed while nothing fired). Readers additionally accept the legacy discriminator as a fallback — in the classifier **and** in the service's rule-question lookup, which had to agree or a class would classify and then yield zero questions.
  - **A second discovered policy class silently disabled rule validation for all of them (#601).** Discovery never sets a matching regex, and validation requires one as soon as there are ≥2 classes, so uploading a second policy manual — the module's own documented "Cross-Policy Coverage" use case — switched off the *first* policy's rules too. The user changed no configuration by hand; the only signal was a WARNING in a downstream Lambda log. The save that creates that state now warns, and the warning reaches the discovery job's status message in the UI rather than only CloudWatch: the job that creates the problem is the right place to report it. The warning covers **both** bad states, not just the all-or-nothing one: a class with no regex sitting alongside classes that have one is never evaluated while the others work, which is the likelier state *after* a user acts on the first warning (add a regex to policy A, run discovery again, and new class B silently can't fire). That matches the stricter rule the new `config_library` preset gate enforces, so the two agree. The runtime's fail-closed behavior is deliberately unchanged (failing open would run every rule against every document) and is now pinned by a test so the warning and the behavior it describes cannot drift.
  - **Rules duplicated across policies were invisible until the bill arrived (#602).** Rule Validation answers each copy with its own LLM call for every document, so a rule stated in three overlapping payer manuals is a standing 3× multiplier on that rule forever — and because each copy is extracted verbatim from its own source, the differently-worded copies can return contradictory Pass/Fail answers with nothing indicating they were the same rule. Saving now reports incoming rule names that already exist in other policy classes, in the log and in the job status message. Name matching (no embeddings) is enough for the common case, since the extraction prompt asks for descriptive `snake_case` identifiers that converge.

  Deliberately out of scope, and still documented as limitations: multi-document clustering / `policy_group` (the token-limit workaround of splitting a manual by chapter still fragments one policy across several classes), LLM-proposed regexes, and automatic dedup. The Limitations section now also spells out that following the documented chapter-split workaround triggers the multiple-class regex requirement. (#600, #601, #602, #603)

- **The configuration version is now pinned once, at the workflow entry point, instead of being re-resolved by each consumer.** `queue_processor` — the single chokepoint every document execution passes through — now stamps `document.config_version` before compressing and starting the Step Functions execution, using a new `ConfigurationManager.resolve_active_version()`. Previously the pin was set only when the uploader supplied `config-version` S3 metadata or when the queue sender managed to resolve it, so a document could reach the workflow unpinned; each downstream consumer then resolved the active version independently, and every one of those independent resolutions was a place the answer could disagree or silently fail. That is precisely how the bug below presented — the queue sender failed to stamp a version, so the pipeline-hooks dispatcher fell back to its own broken scan, read `Config#default`, and disabled every registered hook. Pinning collapses N resolutions into one recorded value, and also guarantees the state machine's `use_bda` / `bda_project_arn` routing flags are read from the *same* version as the rest of the pipeline. Deliberately **not** a new failure mode: an existing pin is never overwritten (so an upload-time choice, or a version carried through a HITL reprocess, still wins), and neither "no version is active" nor a DynamoDB failure fails the document — a freshly deployed stack writes `Config#default` with no `IsActive` attribute at all, so "nothing active" is a normal state that resolves to `default`. The dispatcher's own scan is retained as a defensive fallback for documents queued by an older release or pinned during a DynamoDB outage, and its module docstring now says so explicitly.
...
- **Pipeline hooks silently stopped firing as the ConfigurationTable grew, and three other places resolved the wrong configuration version for the same reason.** Four Lambdas located the active configuration version with a filtered DynamoDB `Scan`. DynamoDB applies both `Limit` and the implicit 1MB page size to the items it **examines**, not the items matching `FilterExpression`, so a single scan call finds nothing whenever the active `Config#` row happens to sort beyond that window — which gets likelier with every configuration version an admin saves and every feature installed. None of the four paginated. The failures were silent and growth-triggered, so they strike long-lived production stacks hardest, and they depend on hash distribution, so two stacks running identical software behave differently. Observed live on a v0.6.3 stack with 35 config rows: the active row sat at scan position 33 behind a 10-item examine window, so **every registered pipeline hook stopped being invoked** — the workflow still succeeded, the document still reached `COMPLETED`, the hook state was entered and exited normally, and the hook Lambda wrote no log entries at all. For a vertical pack that injects business logic via a hook, that means documents are ingested and never decided with every dashboard green. All four sites now page until they match, and each projects only the key attribute so the examine window is not spent reading whole config bodies: the **pipeline-hooks dispatcher** (hooks stop firing), **`registerFeatureHooks`** (hooks written into a non-active version, so registration reports success and they never run — previously `Limit=1`, i.e. broken on any table with more than a handful of versions), the **queue sender** (the document is processed under the *default* configuration rather than the active one), and the **Test Studio test runner** (a run's comparisons are scored against a configuration the documents were not processed under). Two diagnosability fixes alongside, since this hid for over an hour: failing to resolve an active version now logs at **WARNING** and is worded distinctly from the ordinary no-op of a host with no features installed (they previously logged identically, both at INFO), and the dispatcher now reports `configVersion` in its **no-hooks** result too — `invoked: 0` alone cannot distinguish "the active version has no hooks here" from "we resolved the wrong version", which is what made this undiagnosable from the state machine output. See [Feature Platform developer guide → Verifying a hook actually fired](docs/feature-platform-developer-guide.md). (#599)
