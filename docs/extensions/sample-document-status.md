...
This sample is bundled with the accelerator and listed in the catalog. Open
**Extensions → Browse catalog** in the nav and select **Sample: Document Status
(feature add-on)**; an admin installs it from its feature page with one click —
there's no subscription step for open-source extensions. Once installed it gets
its own entry in the **Extensions** nav.
