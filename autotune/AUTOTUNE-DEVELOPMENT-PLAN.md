...
- [x] **Cost observability per optimization run** — Agent token cost (from Strands `accumulated_usage` + `config_library/pricing.yaml`) and eval pipeline cost (from `costBreakdown.totalCost`) tracked separately in DynamoDB, synced every 10s, displayed in frontend status bar. Resume-safe (eval cost seeded from DDB). See `autotune/agent/pricing.py`.
