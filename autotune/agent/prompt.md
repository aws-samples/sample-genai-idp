...
5.  Launch a **validation run** with 1 file first: `run_evaluation(test_set_id, context, config_version='v1', n_files=1)`. Wait for it to complete, then download and inspect the results to confirm evaluation produced valid output (not errors like "I don't see any document"). Only after confirming the config works, launch the full run with `n_files=0` (all files). You only need to do a 1-file validation to confirm the pipeline works and produces reasonable output. After that, always run with `n_files=0` for all subsequent iterations.
