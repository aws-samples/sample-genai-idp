...
class TranslatorConfig:
    ...
    def _validate(self):
        ...
        if not self.model_id or not isinstance(self.model_id, str):
            raise ValueError(f"model_id must be a non-empty string, got: {self.model_id}")
        ...
        if not self.system_prompt or not isinstance(self.system_prompt, str):
            raise ValueError(f"system_prompt must be a non-empty string")
        
        # Validate task_prompt_template
        if not self.task_prompt_template or not isinstance(self.task_prompt_template, str):
            raise ValueError(f"task_prompt_template must be a non-empty string")
        ...
        if "{data_example}" not in self.task_prompt_template:
            raise ValueError("task_prompt_template must contain '{data_example}' placeholder")
        
        # Validate few_shot_examples
        if not isinstance(self.few_shot_examples, list):
            raise ValueError(f"few_shot_examples must be a list, got: {type(self.few_shot_examples)}")
...
class ValueExtractionConfig:
    ...
    def _validate(self):
        ...
        required_placeholders = ["{rule_description}", "{natural_language_rule}", "{parameters_json}", "{data_type}", "{data}"]
        for placeholder in required_placeholders:
            if placeholder not in self.task_prompt_template:
                raise ValueError(f"task_prompt_template must contain '{placeholder}' placeholder")
...
class ConfigLoader:
    ...
    def load(config_path: str) -> Config:
        ...
        missing_fields = [field for field in required_translator_fields if field not in translator_data]
        if missing_fields:
            raise ValueError(
        ...
        missing_fields = [field for field in required_extraction_fields if field not in extraction_data]
        if missing_fields:
            raise ValueError(
        ...
        return Config(
            rule_translator=translator_config,
            value_extraction=extraction_config
        )
