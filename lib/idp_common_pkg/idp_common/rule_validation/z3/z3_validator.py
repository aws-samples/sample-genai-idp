...
from .models import RuleJSON, ValidationResult, Parameter
...
class Z3Validator:
    ...
    def validate(
        ...
        """
        from .models import RuleJSON, RuleWithValues
            ...
            try:
                if param.type == "Int":
                    ...
                    raise ValidationError(
                        ...
                        rule_id=rule_id,
                        context={"parameter_name": param.name, "parameter_type": param.type}
                    )
            except Exception as e:
    ...
    def _add_constraints(
        ...
        for i, constraint_str in enumerate(constraints):
            try:
                ...
                z3_expr = self._parse_smt_constraint(constraint_str, z3_vars, rule_id, i)
                solver.add(z3_expr)
            except ValidationError:
                raise
            except Exception as e:
    ...
    def _apply_smt_operator(self, op: str, args: List[Any]) -> Any:
        ...
        if op == "+":
            if len(args) < 2:
                raise ValueError(f"Operator '+' requires at least 2 arguments, got {len(args)}")
            result = args[0]
            for arg in args[1:]:
                result = result + arg
            return result
            ...
            if len(args) == 1:
                ...
                raise ValueError(f"Operator '-' requires at least 1 argument, got {len(args)}")
            ...
            if len(args) != 2:
                raise ValueError(f"Operator '/' requires exactly 2 arguments, got {len(args)}")
            return args[0] / args[1]
        
        elif op == "mod" or op == "%":
            if len(args) != 2:
                raise ValueError(f"Operator 'mod' requires exactly 2 arguments, got {len(args)}")
            return args[0] % args[1]
        ...
        # Comparison operators
            ...
            if len(args) < 2:
                raise ValueError(f"Operator 'distinct' requires at least 2 arguments, got {len(args)}")
        ...
        # Logical operators
            ...
            if len(args) < 1:
                raise ValueError(f"Operator 'and' requires at least 1 argument, got {len(args)}")
            return z3.And(*args)
        
        elif op == "or":
            if len(args) < 1:
                raise ValueError(f"Operator 'or' requires at least 1 argument, got {len(args)}")
            return z3.Or(*args)
        
        elif op == "not":
            if len(args) != 1:
                raise ValueError(f"Operator 'not' requires exactly 1 argument, got {len(args)}")
            return z3.Not(args[0])
        
        elif op == "implies" or op == "=>":
            if len(args) != 2:
                raise ValueError(f"Operator 'implies' requires exactly 2 arguments, got {len(args)}")
            return z3.Implies(args[0], args[1])
        
        elif op == "ite":
            if len(args) != 3:
                raise ValueError(f"Operator 'ite' requires exactly 3 arguments, got {len(args)}")
    ...
    def _bind_values(
        ...
        for param in parameters:
            ...
            if param_name not in extracted_values or extracted_values[param_name] is None:
                continue
            ...
            try:
                ...
                if param.type == "Int":
                    if isinstance(value, bool):
                        ...
                        raise ValidationError(
                            ...
                            context={"parameter_name": param_name, "expected_type": "Int", "actual_value": value}
                        )
                    ...
                    if isinstance(value, bool):
                        raise ValidationError(
                            ...
                            rule_id=rule_id,
                            parameter_values=extracted_values,
                            context={"parameter_name": param_name, "expected_type": "Real", "actual_value": value}
                        )
                    ...
                    if isinstance(value, bool):
                        ...
                        if value.lower() in ("true", "yes", "1"):
                            ...
                            raise ValidationError(
                                ...
                                rule_id=rule_id,
                                parameter_values=extracted_values,
                                context={"parameter_name": param_name, "expected_type": "Bool", "actual_value": value}
                            )
                    else:
                        raise ValidationError(
                    ...
                    raise ValidationError(
                        ...
                        rule_id=rule_id,
                        parameter_values=extracted_values,
                        context={"parameter_name": param_name, "parameter_type": param.type}
                    )
                ...
                raise ValidationError(
                    ...
                    rule_id=rule_id,
                    parameter_values=extracted_values,
                    context={"parameter_name": param_name, "value": value, "parameter_type": param.type}
                )
            ...
            try:
                ...
                if value is not None:
                    ...
                    if z3.is_int_value(value):
                        ...
                        model[var_name] = float(value.numerator_as_long()) / float(value.denominator_as_long())
