...
class Parameter:
    ...
    def _validate(self):
        ...
        if not self.name or not isinstance(self.name, str):
            raise ValueError(f"Parameter name must be a non-empty string, got: {self.name}")
        ...
        if not isinstance(self.required, bool):
            raise ValueError(f"Parameter 'required' must be a boolean, got: {type(self.required)}")
...
class PathMapping:
    ...
    def from_dict(cls, data: dict) -> 'PathMapping':
        ...
        if "parameter_name" not in data:
            raise ValueError("PathMapping dictionary must contain 'parameter_name' field")
...
class RuleJSON:
    ...
    def _validate(self):
        ...
        if not self.natural_language_rule or not isinstance(self.natural_language_rule, str):
            raise ValueError(
        ...
        if not isinstance(self.constraints, list):
            raise ValueError(f"constraints must be a list, got: {type(self.constraints)}")
        ...
        if not isinstance(self.path_mappings, list):
            raise ValueError(f"path_mappings must be a list, got: {type(self.path_mappings)}")
    ...
    def _validate_constraint_parameters(self):
        ...
        for i, constraint in enumerate(self.constraints):
            ...
            smt_keywords = {
                'and', 'or', 'not', 'implies', 'ite', 'assert', 'declare-const',
    ...
    def from_dict(cls, data: dict) -> 'RuleJSON':
        ...
        required_fields = [
            "rule_id", "version", "description", "natural_language_rule",
            "parameters", "constraints", "path_mappings"
        ]
        
        for field_name in required_fields:
            if field_name not in data:
                raise ValueError(f"RuleJSON dictionary must contain '{field_name}' field")
...
class RuleWithValues:
    ...
    @classmethod
    def from_rule_json(cls, rule_json: 'RuleJSON', extracted_values: Dict[str, Any]) -> 'RuleWithValues':
...
class ValidationResult:
    ...
    def _validate(self):
        ...
        if not isinstance(self.satisfied, bool):
            raise ValueError(f"satisfied must be a boolean, got: {type(self.satisfied)}")
