...
from .exceptions import ValidationSystemError, TranslationError, ExtractionError, ValidationError
...
class ValidationSystem:
    ...
    def extract_with_paths(
        ...
        """
        logger.warning(
            "extract_with_paths() is deprecated. Use extract() instead."
        )
    ...
    def validate_rule(
        ...
        try:
            ...
            logger.info(f"Translation completed in {translation_time:.2f}ms, rule_id: {rule_json.rule_id}")
    ...
    def validate_batch(
        ...
        for i, rule_json in enumerate(rules, 1):
            ...
            try:
                ...
                if stop_on_error:
                    logger.error("Stopping batch validation due to error (stop_on_error=True)")
                    raise ValidationSystemError(
    ...
    def get_summary(self, results: List[ValidationResult]) -> Dict[str, Any]:
        ...
        satisfied_count = sum(1 for r in results if r.satisfied)
        unsatisfied_count = sum(1 for r in results if not r.satisfied and r.outcome != "error")
        error_count = sum(1 for r in results if r.outcome == "error")
        ...
        return {
            ...
            "satisfaction_rate": satisfied_count / total_rules if total_rules > 0 else 0.0
        }
    def validate_with_llm_extraction(
        ...
        try:
            ...
            logger.info(f"Complete LLM extraction validation finished in {total_time:.2f}ms")
