...
class SummarizationService:
    ...
    def process_document(
        ...
        if not normalize_boolean_value(summarization_config.get("enabled", True)):
            ...
            # Attempt to reload the document in case pages were generated by a now-complete upstream process (e.g., OCR).      
            if Status.ASSESSMENT_SKIPPED.value not in document.errors:
        ...
        if not document.sections or getattr(document.sections[0], "classification", None) == Status.CLASSIFICATION_SKIPPED.value:
            # Attempt to reload the document in case sections were generated by a now-complete upstream process (e.g., CLASSIFICATION/EXTRACTION).
            document = Document.from_s3(output_bucket, document.id)
            document.errors.extend(current_errors)
            if not document.sections:
