...
class ExtractionService:
    ...
    def process_document_section(self, document: Document, section_id: str) -> Document:
        ...
        if not normalize_boolean_value(extraction_config.get('enabled', True)):
            ...
            document.errors.append(Status.EXTRACTION_SKIPPED.value)
            return document
        
        # No sections: try to rebuild, else fail or skip
        if not document.sections or Status.CLASSIFICATION_SKIPPED.value in document.errors:
            logger.warning("Document has no sections to extract")
            current_errors = document.errors or []
            # Attempt to reload the document in case sections were generated by a now-complete upstream process (e.g., CLASSIFICATION).
            document = Document.from_s3(os.environ["OUTPUT_BUCKET"], document.id, True)
        ...
        try:
            ...
            if not attributes or not attribute_descriptions.strip():
                ...
                output = {
                    ...
                    "classification": class_label, # assigned for backward compatability to populate UI when decoupled
                }
                s3.write_content(
