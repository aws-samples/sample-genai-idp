...
def test_required_arg_count_is_stable(spec):
    ...
    visible in the diff (99 fields require a non-null arg as of this spec — the
    ...
    estimateReviewEffort the 97th, getAnnotationQueue the 98th, and
    reextractTestSetDocument the 99th)."""
    required = [
        f for f, v in spec["fields"].items() if any(a["non_null"] for a in v["args"])
    ]
    assert len(required) == 99
