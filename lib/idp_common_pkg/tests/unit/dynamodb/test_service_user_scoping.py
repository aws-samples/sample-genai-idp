...
def service(mock_dynamodb_client):
    ...
    with patch(
        "idp_common.dynamodb.service.DynamoDBClient", return_value=mock_dynamodb_client
    ):
...
class TestDocumentUpdateWithUserScoping:
    ...
    def test_update_includes_workflow_status(
        ...
        assert (
            call_args["expression_attribute_values"][":WorkflowStatus"] == "SUCCEEDED"
        )
...
class TestDocumentGetWithUserScoping:
    ...
    def test_returns_none_when_document_not_found(self, service, mock_dynamodb_client):
    ...
    def test_extracts_user_id_from_dynamodb_item(self, service, mock_dynamodb_client):
...
class TestUserIsolation:
    ...
    def test_different_users_get_different_pks(self, service, mock_dynamodb_client):
