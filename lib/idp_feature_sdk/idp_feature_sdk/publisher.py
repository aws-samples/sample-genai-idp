...
Publishes one version of a feature to the seller bucket in the exact layout
the main-stack Lambdas (Phase A) expect:

    s3://<seller-bucket>/features/<featureId>/latest.json
    s3://<seller-bucket>/features/<featureId>/v<version>/template.yaml
    s3://<seller-bucket>/features/<featureId>/v<version>/ui-bundle.js
    s3://<seller-bucket>/features/<featureId>/v<version>/manifest.json
    s3://<seller-bucket>/features/<featureId>/v<version>/sha256.txt
...
class FeaturePublisher:
    """Publishes a feature project to an S3 'seller bucket'."""
    ...
    def publish(
        ...
        *,
        ...
        self.console.log(
            f"[green]✓[/green] Updated s3://{seller_bucket}/{latest_key} "
            f"→ {manifest.version}"
        )

        # 3. Compose helpful URLs
        template_url = self._https_url(
            seller_bucket, region, f"{key_prefix}template.yaml"
        )
        bundle_url = self._https_url(seller_bucket, region, f"{key_prefix}ui-bundle.js")
        manifest_url = self._https_url(
            seller_bucket, region, f"{key_prefix}manifest.json"
        )
        latest_json_url = self._https_url(seller_bucket, region, latest_key)
