...
def find_feature_template() -> Path | None:
    ...
    repository where the template lives at
    ``subscription-features/feature-platform/feature-template/`` relative to the repo
    root. We walk up from the current working directory looking for that
    path; this lets the CLI be invoked from any subdirectory (including the
    repo root itself).
