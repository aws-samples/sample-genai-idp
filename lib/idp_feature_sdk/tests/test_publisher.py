...
def test_publish_uploads_all_artifacts(
    demo_feature_project: Path, seller_bucket: str
) -> None:
    result = FeaturePublisher(demo_feature_project).publish(
        seller_bucket=seller_bucket, region="us-east-1"
    )
    ...
    keys = {
        o["Key"] for o in s3.list_objects_v2(Bucket=seller_bucket).get("Contents", [])
    }
...
def test_latest_json_has_correct_contents(
    ...
    s3 = boto3.client("s3", region_name="us-east-1")
    latest = json.loads(
        s3.get_object(Bucket=seller_bucket, Key="features/demo-feature/latest.json")[
            "Body"
        ].read()
    )
...
def test_manifest_json_mirrors_feature_yaml(
    ...
    mf = json.loads(
        s3.get_object(
            Bucket=seller_bucket, Key="features/demo-feature/v1.2.3/manifest.json"
        )["Body"].read()
    )
...
def test_failed_upload_leaves_latest_json_untouched(
    demo_feature_project: Path, seller_bucket: str, monkeypatch
) -> None:
    ...
    try:
        publisher.publish(seller_bucket=seller_bucket, region="us-east-1")
    except RuntimeError:
        pass
