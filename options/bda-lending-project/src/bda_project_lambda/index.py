...
def handler(event, context):
    ...
    try:
        if request_type == "Create":
            ...
            logger.error(msg)
            cfnresponse.send(
                event, context, cfnresponse.FAILED, {}, physical_id, reason=msg
            )
...
def get_filtered_public_blueprints(blueprint_names=None):
    ...
    try:
        ...
        response = bedrock_client.list_blueprints(
            resourceOwner="SERVICE", maxResults=100
        )
        ...
        for blueprint in response.get("blueprints", []):
            ...
            all_blueprints.append({"arn": blueprint_arn, "name": blueprint_name})
        ...
        if blueprint_names:
            ...
            for blueprint in all_blueprints:
                ...
                if blueprint_name in blueprint_names:
                    ...
                    logger.info(
                        f"Including blueprint: {blueprint_name} ({blueprint['arn']})"
                    )

            logger.info(
                f"Filtered to {len(filtered_blueprints)} blueprints based on provided names"
            )

            # If we still have no matches, log more details for debugging
            if not filtered_blueprints:
                logger.warning(
                    "No matching blueprints found. Available blueprint names:"
                )
                for blueprint in all_blueprints:
...
def create_or_update_custom_blueprint(project_name):
    ...
    try:
        ...
        try:
            response = bedrock_client.list_blueprints(
                resourceOwner="ACCOUNT", maxResults=100
            )
        ...
        if existing_blueprint_arn:
            try:
                ...
                logger.info(
                    f"Updated custom blueprint with ARN: {updated_blueprint_arn}"
                )

                # Return the blueprint config for project creation/update
                return {"blueprintArn": updated_blueprint_arn, "blueprintStage": "LIVE"}
            except Exception as e:
        ...
        # Return the blueprint config for project creation/update
        return {"blueprintArn": new_blueprint_arn, "blueprintStage": "LIVE"}
...
def get_project_config(
    project_description, all_blueprint_configs, project_name=None, project_arn=None
):
    ...
    project_config = {
        ...
        "customOutputConfiguration": {"blueprints": all_blueprint_configs},
...
def create_project(event):
    ...
    project_description = properties.get(
        "ProjectDescription", "Project for processing lending package documents"
    )
    blueprint_names = properties.get("BlueprintNames")
    create_custom_blueprint = (
        properties.get("CustomHomeApplicationBlueprint", "false").lower() == "true"
    )
    ...
    try:
        ...
        response = bedrock_client.list_data_automation_projects(
            resourceOwner="ACCOUNT", maxResults=100
        )
        ...
        for project in response.get("projects", []):
            if project.get("projectName") == project_name:
                ...
                logger.info(
                    f"Found existing project with name '{project_name}' and ARN: {existing_project_arn}"
                )
                break
    ...
    if existing_project_arn:
        logger.info(
            f"Project '{project_name}' already exists, updating instead of creating"
        )
    ...
    project_config = get_project_config(
        project_description, all_blueprint_configs, project_name
    )
    project_config.pop("projectArn")
    logger.info(
        f"Creating project with {len(all_blueprint_configs)} blueprints - config: {json.dumps(project_config)}"
    )
    ...
    return {"projectArn": project_arn, "blueprintArns": blueprint_arns}
...
def update_project(event, project_arn):
    ...
    project_name = properties.get("ProjectName")
    project_description = properties.get(
        "ProjectDescription", "GenAI IDP Sample project"
    )
    blueprint_names = properties.get("BlueprintNames")
    create_custom_blueprint = (
        properties.get("CustomHomeApplicationBlueprint", "false").lower() == "true"
    ...
    update_config = get_project_config(
        project_description, all_blueprint_configs, project_arn=project_arn
    )
