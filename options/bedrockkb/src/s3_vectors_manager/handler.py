...
def lambda_handler(event, context):
    ...
    try:
        ...
        try:
            ...
            try:
                fallback_physical_id = event.get(
                ...
                cfnresponse.send(
                    ...
                    cfnresponse.FAILED,
                    {},
                    physicalResourceId=fallback_physical_id,
                )
                response_sent = True
            except Exception as final_error:
                logger.error(
                    f"FINAL FAILURE: Cannot send basic CFN response: {str(final_error)}"
                )
...
def handle_s3_vector_resources(event, context, properties):
    ...
    metadata_configuration = properties.get(
    ...
    logger.info(
        f"Raw bucket name: {raw_bucket_name}, Sanitized bucket name: {bucket_name}"
    )
    logger.info(
        f"Index properties - Dimension: {vector_dimension}, Metric: {distance_metric}, Metadata: {metadata_configuration}"
    )
    ...
    if request_type == "Create":
        ...
        return create_s3_vector_resources(
            s3vectors_client,
            bucket_name,
            index_name,
            embedding_model,
        ...
        if old_bucket_name != bucket_name or old_index_name != index_name:
            if old_bucket_name and old_index_name:
                delete_s3_vector_resources(
                    s3vectors_client, old_bucket_name, old_index_name
                )
            return create_s3_vector_resources(
                s3vectors_client,
                bucket_name,
            ...
            return update_s3_vector_info(
                s3vectors_client, bucket_name, index_name, index_config
            )
...
def extract_knowledge_base_id_from_physical_id(physical_id):
    ...
    try:
        ...
        logger.warning(
            f"Could not extract KB ID from PhysicalResourceId {physical_id}: {e}"
        )
        return None


def create_knowledge_base_s3_vectors(
    bedrock_agent_client,
    name,
    description,
    role_arn,
    ...
    try:
        ...
        if kms_key_arn:
            logger.info(
                f"KMS encryption configured at bucket level with key: {kms_key_arn}"
            )
...
def delete_vector_index(s3vectors_client, bucket_name, index_name):
    ...
    try:
        ...
        if e.response["Error"]["Code"] in [
            ...
            logger.info(
                f"Index {index_name} not found (already deleted or never existed)"
            )
            return False
        else:
...
def create_vector_index(
    ...
    try:
        ...
        logger.info(
            f"Index configuration - Dimension: {dimension}, Distance Metric: {distance_metric}, Metadata Config: {metadata_configuration}"
        )
        ...
        if e.response["Error"]["Code"] == "ConflictException":
            ...
            logger.info(
                f"Index {index_name} already exists or was created by another process"
            )
            return None
        else:
...
def create_s3_vector_resources(
    s3vectors_client,
    bucket_name,
    index_name,
    embedding_model,
    ...
    try:
        ...
        create_bucket_params = {"vectorBucketName": bucket_name}
        ...
        if bucket_name:
            try:
                ...
                if e.response["Error"]["Code"] not in [
                    ...
                    logger.info(
                        f"Vector bucket {bucket_name} not found or not empty (expected): {e}"
                    )
...
def get_s3_vector_info(s3vectors_client, bucket_name, index_name, index_config=None):
    ...
    try:
        # Get bucket info
        bucket_response = s3vectors_client.get_vector_bucket(
            vectorBucketName=bucket_name
        )
        ...
        if index_config:
            ...
            index_created = create_vector_index(
                s3vectors_client, bucket_name, index_name
            )
