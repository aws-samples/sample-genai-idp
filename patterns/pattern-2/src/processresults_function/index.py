...
def get_confidence_threshold_from_config(document: Document) -> float:
    ...
    try:
        ...
        if threshold_value < 0.0 or threshold_value > 1.0:
            logger.warning(
                f"Invalid confidence threshold value {threshold_value}. Must be between 0.0 and 1.0. Using default: 0.70"
            )
            return 0.70

        logger.info(
            f"Retrieved confidence threshold from configuration: {threshold_value}"
        )
        return threshold_value
    except Exception as e:
        logger.warning(
            f"Failed to retrieve confidence threshold from configuration: {e}"
        )
...
def start_human_loop(
    ...
    logger.info(
        f"Starting A2I for section {section_id}, page {page_ids[0] if page_ids else 'unknown'}"
    )
    ...
    logger.info(
        f"Human loop input structure: {json.dumps(human_loop_input, indent=2, default=str)}"
    )
...
def extract_all_fields_from_explainability(
    explainability_info: list, page_id: str
) -> list:
    """
    ...
    def extract_fields_recursive(
        data: dict, current_path: str = "", page_filter: str = None
    ):
        for key, value in data.items():
            ...
            if isinstance(value, dict):
                ...
                if "confidence" in value:
                    ...
                    if (
                        page_filter is None
                        or field_page == page_filter
                        or field_page == str(page_filter)
                    ):
                        fields.append(
                            {
                                "name": new_path,
...
def process_section_for_hitl(
    ...
    logger.info(
        f"Section {section.section_id} has {len(section.confidence_threshold_alerts)} confidence threshold alerts"
    )
    ...
    output_bucket = (
        uri_parts[2] if len(uri_parts) >= 3 else os.environ.get("OUTPUT_BUCKET", "")
    )
    ...
    for page_id in section.page_ids:
        logger.info(
            f"Processing page {page_id} for section {section.section_id} (section page number: {section_page_number})"
        )

        # Extract all fields for this specific page using section-relative page number
        page_fields = extract_all_fields_from_explainability(
            explainability_info, str(section_page_number)
        )
        ...
        if page_fields:
            ...
            fields_with_threshold = sum(
                1
                for f in page_fields
                if f.get("confidence_threshold") is not None
                and f.get("confidence_threshold") != 0
            )
            fields_without_threshold = len(page_fields) - fields_with_threshold
            logger.info(
                f"Page {page_id}: {fields_with_threshold} fields with specific thresholds, {fields_without_threshold} using config threshold ({confidence_threshold})"
            )
            ...
            for field in page_fields:
                ...
                if (
                    field_confidence_threshold is None
                    or field_confidence_threshold == 0
                ):
                    confidence_threshold_val = (
                        confidence_threshold  # Use config threshold
                    )
                    logger.debug(
                        f"Field {field_name} using config threshold: {confidence_threshold}"
                    )
                else:
                    confidence_threshold_val = field_confidence_threshold
                    logger.debug(
                        f"Field {field_name} using field-specific threshold: {confidence_threshold_val}"
                    )
            ...
            logger.warning(
                f"No fields found for page {page_id} in section {section.section_id}"
            )
            ...
            bounding_boxes.append(
                {
                    "key": "page_review_required",
                    "bounding_box": {"top": 0, "left": 0, "width": 1.0, "height": 1.0},
                }
            )
        ...
        source_image_uri = (
            f"s3://{output_bucket}/{document_id}/pages/{page_id}/image.jpg"
        )
        ...
        try:
            ...
            logger.info(
                f"Successfully triggered A2I for section {section.section_id}, page {page_id} (section page {section_page_number})"
            )
            any_hitl_triggered = True
        except Exception as e:
            logger.error(
                f"Failed to trigger A2I for section {section.section_id}, page {page_id}: {str(e)}"
            )
        ...
        if section_document:
            ...
            if section_document.sections:
                ...
                if enable_hitl == "true" and section.confidence_threshold_alerts:
                    logger.info(
                        f"Checking A2I trigger for section {section.section_id} with {len(section.confidence_threshold_alerts)} alerts"
                    )
                    logger.info(
                        f"Processing section {section.section_id} for HITL with confidence threshold {confidence_threshold}"
                    )
                    logger.info(
                        f"Section confidence threshold alerts: {section.confidence_threshold_alerts}"
                    )
                    ...
                    try:
                        ...
                        section_obj = s3_client.get_object(
                            Bucket=section_bucket, Key=section_key
                        )
                        section_data = json.loads(
                            section_obj["Body"].read().decode("utf-8")
                        )
                        ...
                        logger.error(
                            f"Error processing A2I for section {section.section_id}: {str(e)}"
                        )
