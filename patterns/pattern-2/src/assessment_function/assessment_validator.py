...
class AssessmentValidator:
    ...
    def check_missing_explainability(self):
        ...
        return {"is_valid": True, "failed_attributes": [], "error_message": None}
    ...
    def check_explainability_count(self):
        ...
        for attr_name in self.inference_result:
            if attr_name in assessments:
                ...
                if isinstance(inference_attr, list) and isinstance(
                    assessment_attr, list
                ):
                    ...
                    if inference_count != assessment_count:
                        ...
                        incomplete_sections.append(
                            f"{attr_name}: {missing_count} items not assessed"
                        )
    ...
    def check_explainability_exists(self):
        ...
        expected_attributes = set(self.inference_result.keys())
        assessment_enabled = normalize_boolean_value(
            self.assessment_config.get("enabled", False)
        )
        logger.info(f"Check Explainability Exists: {assessment_enabled}")
        if (
            assessment_enabled
            and expected_attributes
            and (not self.explainability_info or not self.explainability_info[0])
        ):
            return {
                "is_valid": False,
    ...
    def validate_all(self):
        ...
        if self.enable_missing_check:
            ...
            if not missing_result["is_valid"]:
                ...
                validation_results["failed_attributes"].extend(
                    missing_result["failed_attributes"]
                )
        ...
        if self.enable_count_check:
            ...
            if not count_result["is_valid"]:
                validation_results["is_valid"] = False
                validation_results["failed_attributes"].extend(
                    count_result["failed_attributes"]
                )
                validation_results["validation_errors"].append(
        ...
        if not exists_result["is_valid"]:
            validation_results["is_valid"] = False
            validation_results["failed_attributes"].extend(
                exists_result["failed_attributes"]
            )
            validation_results["validation_errors"].append(
                exists_result["error_message"]
            )

        # Log final validation result
        if validation_results["is_valid"]:
            logger.info(
                "Assessment validation: all or some attributes have confidence scores"
            )
        else:
            logger.info(
                f"Assessment validation: {len(validation_results['failed_attributes'])} issues found"
            )
...
class AssessmentFileValidator:
    ...
    def load_and_validate(self):
        ...
        validator = AssessmentValidator(
            data,
            self.enable_missing_check,
            self.enable_count_check,
