...
def handler(event, context):
    ...
    try:
        ...
        logger.info(
            f"Processing document with SummarizationService, document ID: {document.id}"
        )
        ...
        # Log the result
        if (
            hasattr(processed_document, "summary_report_uri")
            and processed_document.summary_report_uri
        ):
            logger.info(
                f"Document summarization successful, report URI: {processed_document.summary_report_uri}"
            )
        else:
            logger.warning(
                "Document summarization completed but no summary report URI was set"
            )

        # Add Lambda metering for successful summarization execution
        try:
            lambda_metering = calculate_lambda_metering(
                "Summarization", context, start_time
            )
            processed_document.metering = merge_metering_data(
                processed_document.metering, lambda_metering
            )
        ...
        return {
            "document": processed_document.serialize_document(
                working_bucket, "summarization", logger
            ),
        }
        ...
        try:
            if "document" in locals() and document:
                ...
                document_service = create_document_service()
                logger.info(
                    f"Updating document status to {document.status} due to error"
                )
                document_service.update_document(document)
        except Exception as status_error:
            logger.error(
                f"Failed to update document status: {str(status_error)}", exc_info=True
            )
