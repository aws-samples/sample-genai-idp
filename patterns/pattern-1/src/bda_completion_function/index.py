...
def get_task_token(object_key: str) -> str:
    try:
        ...
        response = tracking_table.get_item(
            Key={"PK": key, "SK": "none"}, ConsistentRead=True
        )
    ...
    try:
        if job_status == "SUCCESS":
            ...
            stepfunctions.send_task_success(
                ...
                output=json.dumps({"status": "SUCCESS", "job_detail": job_detail}),
            )
...
def handler(event, context):
    ...
    try:
        ...
        logger.info(
            f"Processing job completion for object: {object_key}, status: {job_status}"
        )
        ...
        response = {"statusCode": 200, "body": "Task response sent successfully"}
