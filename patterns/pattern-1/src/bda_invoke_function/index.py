...
def invoke_data_automation(
    data_project_arn: str, input_s3_uri: str, output_s3_uri: str
) -> Dict[str, Any]:
    retry_count = 0
    last_exception = None
    ...
    bda_service = BdaService(
        dataAutomationProjectArn=data_project_arn, output_s3_uri=output_s3_uri
    )
    ...
    while retry_count < MAX_RETRIES:
        try:
            ...
            logger.info(
                f"BDA API request successful after {retry_count + 1} attempts. "
            ...
            metrics.put_metric(
                "BDARequestsTotalLatency", total_duration * 1000, "Milliseconds"
            )
            ...
            if error_code in retryable_errors:
                ...
                backoff = utils.calculate_backoff(
                    retry_count, INITIAL_BACKOFF, MAX_BACKOFF
                )
                logger.warning(
                    f"BDA API throttling occurred (attempt {retry_count}/{MAX_RETRIES}). "
                ...
                logger.error(
                    f"Non-retryable BDA API error: {error_code} - {error_message}"
                )
                metrics.put_metric("BDARequestsFailed", 1)
...
def handler(event: Dict[str, Any], context: Any) -> Dict[str, Any]:
    try:
        ...
        logger.info(
            f"API invocation successful. Response: {json.dumps(bda_response, default=str)}"
        )
        return response
