...
def get_confidence_threshold_from_config(document: Document) -> float:
    ...
    try:
        ...
        if threshold_value < 0.0 or threshold_value > 1.0:
            logger.warning(
                f"Invalid confidence threshold value {threshold_value}. Must be between 0.0 and 1.0. Using default: 0.80"
            )
            return 0.80
...
def start_human_loop(
    ...
    try:
        ...
        project_response = bedrock_client.get_data_automation_project(
            projectArn=os.environ.get("BDA_PROJECT_ARN", ""), projectStage="LIVE"
        )
        ...
        for blueprint_arn in blueprint_arns:
            try:
                ...
                blueprint_response = bedrock_client.get_blueprint(
                    blueprintArn=arn_string, blueprintStage="LIVE"
                )
        ...
        logger.info(
            f"Retrieved {len(blueprint_options)} blueprint options from project"
        )
    except ClientError as e:
        logger.error(
            f"Failed to retrieve Bedrock project: {e.response['Error']['Message']}"
        )
        blueprint_options = []
    except Exception as e:
    ...
    try:
        FlowDefinitionArn = ssm.get_parameter(
        ...
        if segment.get("custom_output_status") == "MATCH":
            ...
            if enable_hitl == "true":
                low_confidence = (
                    any(
                        ...
                        for kv in pagespecific_details["key_value_details"].get(
                            str(page_num), []
                        )
                    )
            ...
            if low_confidence:
                ...
                for page_number in page_indices:
                    ...
                    key_values = pagespecific_details["key_value_details"].get(
                        page_str, []
                    )
                    bounding_boxes = pagespecific_details["bounding_box_details"].get(
                        page_str, []
                    )
                    ...
                    try:
                        ...
                        logger.info(
                            f"Started human loop for page {page_number}: {human_loop_response}"
                        )
                    except Exception as e:
                        logger.error(
                            f"Failed to start human loop for page {page_number}: {str(e)}"
                        )
