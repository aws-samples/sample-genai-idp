...
def train(
    ...
    tb_logger = TensorBoardLogger(
        save_dir=os.path.join(output_dir, "tensorboard"), name="training_logs"
    )
    # Load processor with pinned revision for security (addresses B615 finding)
    revision = (
        get_model_revision(base_model)
        if base_model in ["microsoft/udop-large"]
        else None
    )
    if revision:
        print(f"Loading processor for {base_model} with pinned revision: {revision}")
        processor = AutoProcessor.from_pretrained(
            base_model, revision=revision, apply_ocr=False
        )
    else:
        # Fallback for custom models without managed versions
        print(
            f"Loading processor for {base_model} without revision pinning (not in managed list)"
        )
    ...
    assert (
        train_ds.prompt.strip() == val_ds.prompt.strip()
    ), "Prompts do not match in training and validation dataset!\nTraining Prompt: {0}\nValidation Prompt: {1}".format(  # nosec B101
        train_ds.prompt, val_ds.prompt
    )

    train_dl = DataLoader(
        train_ds, batch_size=1, num_workers=4, collate_fn=lambda x: x[0], shuffle=True
    )
    val_dl = DataLoader(
        val_ds, batch_size=1, num_workers=4, collate_fn=lambda x: x[0], shuffle=True
    )

    max_steps = (max_epochs * len(train_ds)) // accumulate_grad_batches // devices
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--distributed_strategy", type=str, default="ddp_find_unused_parameters_true"
    )
