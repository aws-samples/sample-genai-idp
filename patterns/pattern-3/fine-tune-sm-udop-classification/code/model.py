...
class UDOPModel(pl.LightningModule):
    def __init__(
        ...
        revision = (
            get_model_revision(model_id)
            if model_id in ["microsoft/udop-large"]
            else None
        )
        if revision:
            ...
            print(
                f"Loading model {model_id} without revision pinning (not in managed list)"
            )
            self.model = UdopForConditionalGeneration.from_pretrained(
    ...
    def on_train_epoch_end(self):
        d = {
            t: {
                ...
                "targets": [
                    o["targets"] for o in self.training_step_outputs if o["task"] == t
                ],
            }
    ...
    def on_validation_epoch_end(self):
        ...
        d = {
            t: {
                ...
                "targets": [
                    o["targets"] for o in self.validation_step_outputs if o["task"] == t
                ],
            }
            for t in self.tasks.keys()
        }
    ...
    def configure_optimizers(self):
        ...
        ]  # Update LR in each step, not epoch
