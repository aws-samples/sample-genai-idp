...
def create_deployment_role(role_name, model_bucket):
    ...
    try:
        ...
        iam.create_policy(PolicyName=policy_name, PolicyDocument=json.dumps(s3_policy))
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--model-artifact", type=str, required=True, help="S3 uri for the model.tar.gz"
    )
    parser.add_argument(
        "--endpoint-name", type=str, required=True, help="Endpoint name"
    )
    parser.add_argument("--base-model", type=str, default="microsoft/udop-large")
    args = parser.parse_args()
    deploy_model(args.role, args.base_model, args.model_artifact, args.endpoint_name)
