...
mapping_split = {"train": "training", "test": "validation"}
...
def save_image_2s3(client, image_bytes, bucket_name, keystr):
    ...
    client.put_object(
        Bucket=bucket_name, Key=keystr, Body=image_bytes, ContentType="image/png"
    )
...
def process_each(data, textract_client):
    ...
    return {"image": image_bytes, "label": label, "textract": textract_json}
...
def process_and_save(
    data, idx, bucket_name, prefix, split, textract_client, s3_client, logger
):
...
def metadata(client, data, bucket_name, prefix, split):
    ...
    meta_data = {"labels": mapping, "size": len(data), "name": "RVLCDIP"}
    save_json_2s3(
        client,
        meta_data,
        bucket_name,
...
def generate(split, bucket_name, prefix, max_workers):
    ...
    textract_client = boto3.client(
        "textract", region_name=region, config=adaptive_config
    )
    ...
    with concurrent.futures.ThreadPoolExecutor(max_workers=max_workers) as executor:
        ...
        for future in tqdm(
            ...
            total=len(data),
            desc=f"Processing {mapping_split[split]} data",
            unit="item",
        ):
            try:
    ...
    logger.info(
        f"Data is saved under 's3://{bucket_name}/{prefix}/{mapping_split[split]}'"
    )


if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Process and upload dataset to S3 with AWS Textract."
    )
    parser.add_argument(
        "--data-bucket",
        type=str,
        required=True,
        help="Name of the S3 bucket for storing data.",
    )
    parser.add_argument(
        "--data-bucket-prefix", type=str, default="", help="Prefix for S3 bucket paths."
    )
    parser.add_argument(
        "--max-workers", type=int, default=40, help="Number of parallel workers."
    )
