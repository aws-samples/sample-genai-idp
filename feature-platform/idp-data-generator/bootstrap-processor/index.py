...
def _register_test_set(test_set_id, status, document_class_type=None):
    """Upsert the host test-set registration record so the set shows in the
    Test Studio list (getTestSets reads ItemType='testset' records)."""
    ...
    if document_class_type:
        attrs["documentClassType"] = document_class_type
    try:
        _ddb.Table(HOST_TRACKING_TABLE).put_item(
            Item={"PK": f"testset#{test_set_id}", "SK": "metadata", **attrs}
        )
...
def _process_job(job_id, body):
    ...
    _register_test_set(target_version, "GENERATING")
    _status(
        job_id,
        "IN_PROGRESS",
