...
#
# VENDORED from lib/idp_common_pkg/idp_common/delete_documents.py.
# A feature builds with a plain `sam build` that copies only this hook/ dir to a
# temp build context, so a repo-relative dependency on idp_common cannot resolve.
# This module is boto3/stdlib-only, so it is copied here verbatim. Keep it in
# sync with the host copy if the TrackingTable schema / shard scheme changes
# (the sync-pii-anonymizer skill notes this).
