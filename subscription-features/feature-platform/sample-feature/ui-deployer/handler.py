...
On Create or Update:
  1. Copies s3://<SELLER_BUCKET>/features/<FEATURE_ID>/v<FEATURE_VERSION>/ui-bundle.js
...
_SELLER_BUCKET = os.environ["SELLER_BUCKET"]
...
if _FEATURE_VERSION == "<FEATURE_VERSION_TOKEN>" or "TOKEN" in _FEATURE_VERSION:
    raise RuntimeError(
        ...
        f"This means the seller-bucket template.yaml still contains the "
...
def _bundle_ui(request_type: str) -> str:
    ...
    src_key = f"features/{_FEATURE_ID}/v{_FEATURE_VERSION}/ui-bundle.js"
    dst_key = src_key  # same layout inside WebUIBucket
    ...
    if request_type in ("Create", "Update"):
        ...
        _s3.copy_object(
            CopySource={"Bucket": _SELLER_BUCKET, "Key": src_key},
