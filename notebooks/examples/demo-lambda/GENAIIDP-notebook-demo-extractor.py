...
def lambda_handler(event, context):
    ...
    try:
        ...
        logger.info(
            f"Temperature: {extraction_config.get('temperature', 'Not specified')}"
        )
        logger.info(
            f"Max tokens: {extraction_config.get('max_tokens', 'Not specified')}"
        )
        logger.info(
            f"Custom Lambda ARN: {extraction_config.get('custom_prompt_lambda_arn', 'Not specified')}"
        )

        # Default system prompt from config
        default_system_prompt = config.get("extraction", {}).get("system_prompt", "")
        logger.info(
            f"Default system prompt length: {len(default_system_prompt)} characters"
        )
        ...
        if "bank statement" in document_class.lower():
            ...
            result = _handle_bank_statement(
                placeholders, default_system_prompt, document_text
            )
            ...
            result = _handle_generic_document(
                placeholders, default_system_prompt, default_content
            )
        ...
        logger.info(
            f"System prompt (first 200 chars): {result.get('system_prompt', '')[:200]}..."
        )
...
def _handle_bank_statement(placeholders, default_system_prompt, document_text):
    ...
    is_business = any(
        indicator in document_text.lower() for indicator in business_indicators
    )
...
def _handle_invoice(placeholders, default_system_prompt, document_text):
    ...
    is_international = any(
        indicator in document_text.lower() for indicator in international_indicators
    )
