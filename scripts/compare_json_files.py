...
class ComparisonError:
    ...
    def to_dict(self):
        return {"error_type": self.error_type, "message": str(self.message)}
    ...
    try:
        ...
        if not content.strip():
            return ComparisonError(
                "empty_content", f"Empty content in file: {file_key}"
            )
                ...
                if diff:
                    ...
                    for change_type, changes in diff.items():
                        if change_type in ["values_changed", "type_changes"]:
                            for path in changes:
                                field = (
                                    path.split("[")[0].replace("root", "").strip(".")
                                )
...
def get_ai_summary(comparison_results, summary_stats, field_differences):
    ...
    bedrock = boto3.client(
        service_name="bedrock-runtime", config=Config(region_name="us-east-1")
    )
    ...
    field_diff_summary = "\n".join(
        [
            f"{field}: {count} differences"
            for field, count in sorted(
                field_differences.items(), key=lambda x: x[1], reverse=True
            )
        ]
    )
    ...
    try:
        ...
        response = bedrock.invoke_model(
            modelId="eu.anthropic.claude-3-5-sonnet-20240620-v1:0", body=body
        )
...
def generate_report(
    comparison_results, summary_stats, field_differences, invalid_files, output_dir="."
):
    """
    ...
    with open(report_path, "w") as f:
        ...
        for result in comparison_results:
            ...
            if result["error"]:
                f.write(
                    f"Error: {result['error']['error_type']} - {result['error']['message']}\n"
                )
...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Compare JSON files between two S3 folders"
    )
...
def main():
    ...
    comparison_results, summary_stats, field_differences, invalid_files = (
        compare_json_sets(args.bucket, args.folder1, args.folder2)
    )
