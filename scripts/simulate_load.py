...
def log_progress(stats, target_rate):
    while True:
        ...
        print(
            f"[{minutes:02d}:{seconds:02d}] Files copied: {total}, Current rate: {rate:.1f} files/minute (target: {target_rate})"
        )
        time.sleep(
            10
        )  # semgrep-ignore: arbitrary-sleep - Intentional delay for periodic logging. Duration is hardcoded and not user-controlled.
...
def stress_test(
    ...
    log_thread = threading.Thread(
        target=log_progress, args=(stats, copies_per_minute), daemon=True
    )
    log_thread.start()
    ...
    with ThreadPoolExecutor(max_workers=max_workers) as executor:
        while time.time() < end_time:
            ...
            time.sleep(
                0.1
            )  # semgrep-ignore: arbitrary-sleep - Intentional delay to avoid tight loop. Duration is hardcoded and not user-controlled.
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "-s", "--source-bucket", required=True, help="Source bucket name"
    )
    parser.add_argument("-k", "--source-key", required=True, help="Source file key")
    parser.add_argument(
        "-d", "--dest-bucket", required=True, help="Destination bucket name"
    )
    parser.add_argument(
        "-p",
        "--dest-prefix",
        default="simulated_load",
        help="Destination prefix (folder)",
    )
    parser.add_argument(
        "-r", "--rate", type=int, default=2500, help="Copies per minute (default: 2500)"
    )
    parser.add_argument(
        "-t", "--duration", type=int, default=1, help="Duration in minutes"
    )
