"""Unit tests for the manual probe runner (scripts/sdlc/run_probe.py).
...
def test_list_returns_zero(rp, monkeypatch, capsys):
    monkeypatch.setattr(sys, "argv", ["run_probe.py", "--list"])
    rc = rp.main()
...
def test_unknown_probe_errs(rp, monkeypatch):
    monkeypatch.setattr(sys, "argv", ["run_probe.py", "nope"])
    assert rp.main() == 2


def _fake_probes(rp, monkeypatch, suffix, validate_fn, requires_vpc=False):
    """Replace _probes_by_suffix with one fake Probe (namedtuple is immutable)."""
    fake = rp.cbd.Probe(
        name=f"fake-{suffix}",
        stack_suffix=suffix,
        deploy_params={},
    ...
    monkeypatch.setattr(rp, "_probes_by_suffix", lambda: {suffix: fake})
    return fake
...
def test_validate_existing_stack_runs_validator_only(rp, monkeypatch):
    ...
    monkeypatch.setattr(
        sys, "argv", ["run_probe.py", "zapdast", "--stack-name", "idp-live"]
    )
...
def test_vpc_probe_without_wiring_errors(rp, monkeypatch):
    ...
    monkeypatch.setattr(
        sys, "argv", ["run_probe.py", "jobsapi", "--stack-name", "idp-live"]
    )
...
def test_self_deploy_requires_template_url(rp, monkeypatch):
    ...
    monkeypatch.setattr(sys, "argv", ["run_probe.py", "zapdast"])
    assert rp.main() == 2
