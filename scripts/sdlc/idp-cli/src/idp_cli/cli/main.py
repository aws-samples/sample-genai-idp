...
def install(
    ...
    cfn_prefix: str = typer.Option(
        "idp-dev", "--cfn-prefix", help="An identifier to prefix the stack"
    ),
    admin_email: str = typer.Option(..., "--admin-email", help="The admin email"),
    idp_pattern: str = typer.Option(
        "Pattern1 - Packet or Media processing with Bedrock Data Automation (BDA)",
        "--idp-pattern",
        help="The IDP Pattern to install",
    ),
    ...
    service = InstallService(
        account_id=account_id, cfn_prefix=cfn_prefix, cwd=cwd, debug=debug
    )
    if publish:
        service.publish()
...
def uninstall(
    stack_name: str = typer.Option(
        ..., "--stack-name", help="Name of the stack to uninstall"
    ),
    account_id: str = typer.Option(..., "--account-id", help="AWS Account ID"),
    cfn_prefix: str = typer.Option(
        "idp-dev", "--cfn-prefix", help="An identifier to prefix the stack"
    ...
    try:
        ...
        service = UninstallService(
            stack_name=stack_name, account_id=account_id, cfn_prefix=cfn_prefix
        )
...
def smoketest(
    stack_name: str = typer.Option(
        "idp-Stack", "--stack-name", help="Name of the deployed stack to test"
    ),
    ...
    verify_string: str = typer.Option(
        "ANYTOWN, USA 12345",
        "--verify-string",
        help="String to verify in the processed output",
    ),
):
    """
    ...
    try:
        ...
        service = SmokeTestService(
            stack_name=stack_name, file_path=file_path, verify_string=verify_string
        )
...
def monitor_pipeline(
    pipeline_name: str = typer.Option(
        ..., "--pipeline-name", help="Name of the CodePipeline to monitor"
    ),
    execution_id: str = typer.Option(
        None, "--execution-id", help="Specific execution ID to monitor"
    ),
    initial_wait: int = typer.Option(
        10, "--initial-wait", help="Initial wait time in seconds before monitoring"
    ),
    poll_interval: int = typer.Option(
        30, "--poll-interval", help="Time in seconds between status checks"
    ),
    ...
    try:
        ...
        try:
            log_messages = CodePipelineUtil.get_stage_logs(
                pipeline_name=pipeline_name, stage_name="Build"
            )
            typer.echo("---\nCodebuild logs:")
