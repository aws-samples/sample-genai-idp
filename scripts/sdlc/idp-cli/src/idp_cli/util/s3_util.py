...
class S3Util:
    ...
    @staticmethod
    def query_json_objects_stream(
        bucket_name: str, sql_expression: str, path: str = ""
    ):
        for result in S3Util.query_json_objects(
            bucket_name=bucket_name, sql_expression=sql_expression, path=path
        ):
    ...
    def query_json_objects(bucket_name: str, sql_expression: str, path: str = ""):
        ...
        s3_client = boto3.client("s3")
        objects = s3_client.list_objects_v2(Bucket=bucket_name, Prefix=path).get(
            "Contents", []
        )
            ...
            try:
                ...
                for event in response["Payload"]:
                    if "Records" in event:
                        ...
                        logger.debug(
                            f"Bytes Scanned: {stats['BytesScanned']}, Bytes Returned: {stats['BytesReturned']}"
                        )
    ...
    @staticmethod
    def query_jsonl_objects_stream(
        bucket_name: str, sql_expression: str, path: str = ""
    ):
        for result in S3Util.query_jsonl_objects(
            bucket_name=bucket_name, sql_expression=sql_expression, path=path
        ):
    ...
    @staticmethod
    def put_jsonl(
        bucket_name: str, object_name: str, jsonl_data: list, concat: bool = False
    ):
    ...
    def get_jsonl(bucket_name: str, object_name: str):
        ...
        try:
            ...
            if jsonl_text:
                return [
                    json.loads(line) for line in jsonl_text.strip().split("\n") if line
                ]
            else:
            ...
            logger.error(
                f"Failed to retrieve or parse JSONL data from '{object_name}': {e}"
            )
            return []
    ...
    def put_zip(bucket_name: str, object_name: str, relative_path: str):
        ...
        try:
                ...
                with zipfile.ZipFile(temp_zip, "w", zipfile.ZIP_DEFLATED) as zipf:
                    for root, _, files in os.walk(absolute_path):
                        for file in files:
                            ...
                            zipf.write(
                                file_path, os.path.relpath(file_path, absolute_path)
                            )
            ...
            logger.debug(
                f"ZIP archive '{object_name}' uploaded to bucket '{bucket_name}'."
            )
        except Exception as e:
    ...
    @staticmethod
    def get_zip(
        bucket_name: str, object_name: str, local_filename: str = "archive.zip"
    ):
        try:
            ...
            if not zip_data:
                logger.debug(
                    f"Failed to retrieve ZIP archive '{object_name}' from bucket '{bucket_name}'."
                )
                return None
            ...
            logger.error(
                f"Failed to retrieve or extract ZIP archive '{object_name}': {e}"
            )
            return None
    ...
    def empty_bucket(bucket_name: str):
        ...
        try:
            ...
            if is_versioned:
                logger.info(
                    f"Emptying versioned bucket '{bucket_name}' (including all object versions)"
                )
    ...
    def delete_bucket(bucket_name: str, empty_first: bool = True):
        ...
        try:
            ...
            if e.response["Error"]["Code"] == "BucketNotEmpty":
                logger.error(
                    f"Cannot delete non-empty bucket '{bucket_name}'. Set empty_first=True or empty the bucket manually."
                )
            else:
    ...
    def list_objects(
        ...
        try:
            s3_client = boto3.client("s3")
            params = {"Bucket": bucket_name, "MaxKeys": max_keys, "Prefix": prefix}
            ...
            logger.debug(
                f"Listed {len(result['Contents'])} objects from bucket '{bucket_name}' with prefix '{prefix}'"
            )
            return result
