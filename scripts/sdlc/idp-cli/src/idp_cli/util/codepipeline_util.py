...
class CodePipelineUtil:
    ...
    def wait_for_pipeline_execution(
        ...
        if execution_id:
            ...
            logger.info(
                f"Waiting {initial_wait_seconds} seconds for pipeline '{pipeline_name}' to start..."
            )
            ...
            try:
                ...
                if not executions:
                    raise Exception(
                        f"No executions found for pipeline '{pipeline_name}'"
                    )
        ...
        while True:
            ...
            if elapsed_time > max_wait_seconds:
                raise Exception(
                    f"Pipeline '{pipeline_name}' execution timed out after {max_wait_minutes} minutes"
                )

            # Get the specific execution status
            try:
                response = client.get_pipeline_execution(
                    pipelineName=pipeline_name, pipelineExecutionId=target_execution_id
                )
                ...
                logger.info(
                    f"Pipeline '{pipeline_name}' execution {target_execution_id} status: {status}"
                )

                if status == "Succeeded":
                    logger.success(
                        f"Pipeline '{pipeline_name}' execution completed successfully!"
                    )
                    return
                elif status == "Failed":
                    failure_reason = execution.get("statusReason", "No reason provided")
                    message = (
                        f"Pipeline '{pipeline_name}' execution failed: {failure_reason}"
                    )
                    ...
                    logger.info(
                        f"Pipeline '{pipeline_name}' execution was superseded but continues running in parallel mode"
                    )
    ...
    def get_pipeline_logs(
        ...
        try:
            ...
            if not execution_id:
                ...
                if not executions:
                    return {
                        "error": f"No executions found for pipeline '{pipeline_name}'"
                    }
                            ...
                            try:
                                ...
                                for pattern in [pattern1, pattern2, pattern3]:
                                    ...
                                    if match:
                                        ...
                                        logger.info(
                                            f"Extracted build ID from URL: {build_id}"
                                        )
                                        break
                                ...
                                if build_info["builds"]:
                                    ...
                                    stage_logs[current_action_name] = [
                                        f"No build information found for build ID: {build_id}"
                                    ]
                            except Exception as e:
                                logger.error(
                                    f"Error retrieving CodeBuild logs: {str(e)}"
                                )
                                stage_logs[current_action_name] = [
            ...
            raise Exception(
                f"Failed to retrieve logs for pipeline '{pipeline_name}': {str(e)}"
            )
    ...
    def get_stage_logs(pipeline_name, stage_name, execution_id=None, max_lines=1024):
        ...
        if stage_name not in logs_by_stage:
            raise KeyError(
                f"Stage '{stage_name}' not found in logs. Available stages: {', '.join(logs_by_stage.keys())}"
            )
    ...
    def get_execution_id_by_version(pipeline_name, version_id, wait_seconds=30):
        ...
        for attempt in range(retry_count):
            ...
            for execution in executions:
                ...
                if (
                    source_revisions
                    and source_revisions[0].get("revisionId") == version_id
                ):
                    logger.info(
                        f"Found execution {execution['pipelineExecutionId']} for version {version_id}"
                    )
                    return execution["pipelineExecutionId"]

            if attempt < retry_count - 1:
                logger.info(
                    f"Version {version_id} not found, retrying... (attempt {attempt + 1}/{retry_count})"
                )
