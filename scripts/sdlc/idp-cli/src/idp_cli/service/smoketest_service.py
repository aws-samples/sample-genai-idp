...
class SmokeTestService:
    def __init__(self, stack_name: str, file_path: str, verify_string: str):
    ...
    def get_bucket_names(self):
        ...
        logger.debug(
            f"Retrieved bucket names - Input: {input_bucket_name}, Output: {output_bucket_name}"
        )
    ...
    def upload_testfile(self):
        ...
        logger.debug(
            f"Uploading test file to bucket: {input_bucket_name}, key: {file_key}"
        )
        S3Util.put_bytes(
    ...
    def wait_for_processing(self, file_key: str):
        ...
        logger.debug(
            f"Waiting for processing, checking for folder: {file_key}/ in bucket: {output_bucket_name}"
        )
        ...
        for attempt in range(max_attempts):
            try:
                ...
                response = S3Util.list_objects(
                    bucket_name=output_bucket_name, prefix=f"{file_key}/", max_keys=1
                )
                ...
                logger.debug(
                    f"Waiting for processing... Attempt {attempt+1}/{max_attempts}. Error: {str(e)}"
                )
        ...
        logger.error(
            f"Processing timed out after {max_attempts * wait_seconds} seconds"
        )
        raise TimeoutError(
            f"Processing timed out after {max_attempts * wait_seconds} seconds"
        )
    ...
    def verify_result(self, folder_key):
        ...
        logger.debug(
            f"Looking for result file at: s3://{output_bucket_name}/{object_path}"
        )

        try:
            # Get the JSON from S3
            result_json = S3Util.get_json(
                bucket_name=output_bucket_name, object_name=object_path
            )

            # Check if result exists
            if not result_json:
                logger.error(
                    f"Result file not found at: s3://{output_bucket_name}/{object_path}"
                )
            ...
            if (
                self.verify_string
                not in result_json["pages"][0]["representation"]["markdown"]
            ):
                logger.error(
                    f"Text content does not contain expected string: '{self.verify_string}'"
                )
                logger.debug(
                    f"Actual text starts with: '{result_json['pages'][0]['representation']['markdown'][:100]}...'"
                )
                raise ValueError(
                    "Text content does not contain expected verification string"
                )
