...
class UninstallService:
    def __init__(
        self, stack_name: str, account_id: str, cfn_prefix: Optional[str] = "idp-dev"
    ):
    ...
    def delete_service_role_stack(self):
        ...
        try:
            logger.info(
                f"Attempting to delete service role stack: {service_role_stack_name}"
            )
            response = CfnUtil.delete_stack(
                stack_name=service_role_stack_name, wait=True
            )
            logger.info(
                f"Successfully deleted service role stack: {service_role_stack_name}"
            )
            logger.debug(response)
        except Exception as e:
            if "does not exist" in str(e):
                logger.debug(
                    f"Service role stack {service_role_stack_name} does not exist, skipping"
                )
            else:
                logger.error(
                    f"Failed to delete service role stack {service_role_stack_name}: {e}"
                )
    ...
    def delete_permission_boundary_policy(self):
        ...
        try:
            ...
            logger.info(
                f"Attempting to delete permission boundary policy: {policy_arn}"
            )
            iam.delete_policy(PolicyArn=policy_arn)
            logger.info(
                f"Successfully deleted permission boundary policy: {policy_arn}"
            )

        except ClientError as e:
            if e.response["Error"]["Code"] == "NoSuchEntity":
                logger.debug(
                    f"Permission boundary policy {policy_name} does not exist, skipping"
                )
            elif e.response["Error"]["Code"] == "DeleteConflict":
                logger.warning(
                    f"Permission boundary policy {policy_name} is still attached to resources, skipping deletion"
                )
            else:
                logger.error(
                    f"Failed to delete permission boundary policy {policy_name}: {e}"
                )
        except Exception as e:
            logger.error(
                f"Unexpected error deleting permission boundary policy {policy_name}: {e}"
            )
