...
class InstallService:
    ...
    def check_docker_availability(self):
        ...
        try:
            ...
            if result.returncode == 0:
                ...
                logger.warning(
                    f"Docker is not running properly. Error: {result.stderr}"
                )
                return False
    ...
    def publish(self):
        ...
        if not docker_available:
            logger.warning(
                "Docker is not available. Using --use-container=false for SAM build."
            )
        ...
        try:
            ...
            logger.debug(
                f"Running publish command: bash ./publish.sh {self.cfn_bucket_basename} {self.cfn_prefix} {self.region}"
            )
            ...
            logger.info(
                f"Successfully published to {self.cfn_bucket_basename} in {self.region}"
            )
            return True
        ...
        try:
            ...
            if not os.path.exists(template_path):
                raise FileNotFoundError(
                    f"Service role template not found: {template_path}"
                )

            logger.info(
                f"Deploying CloudFormation service role stack: {service_role_stack_name}"
            )
            ...
            service_role_arn = self._get_service_role_arn_from_stack(
                service_role_stack_name
            )
            if service_role_arn:
    ...
    def _get_service_role_arn_from_stack(self, stack_name):
        ...
        try:
            describe_cmd = [
                ...
                stack_name,
                "--query",
                "Stacks[0].Outputs[?OutputKey==`ServiceRoleArn`].OutputValue",
                "--output",
                "text",
            ]
    ...
    def validate_permission_boundary(self, stack_name, boundary_arn):
        ...
        try:
            ...
            for role_name in roles:
                try:
                    ...
                    if "PermissionsBoundary" in role:
                        actual_boundary = role["PermissionsBoundary"][
                            "PermissionsBoundaryArn"
                        ]
                        if actual_boundary == boundary_arn:
                            logger.debug(
                                f"✅ {role_name}: Has correct permission boundary"
                            )
                        else:
                            logger.error(
                                f"❌ {role_name}: Has wrong permission boundary: {actual_boundary}"
                            )
            ...
            if failed_roles:
                logger.error(
                    f"FAILED: {len(failed_roles)} roles do not have the correct permission boundary"
                )
                return False
            else:
                logger.info(
                    f"SUCCESS: All {len(roles)} roles have the correct permission boundary"
                )
                return True
    ...
    def install(self, admin_email: str, idp_pattern: str):
        ...
        try:
            ...
            if not permission_boundary_arn:
                logger.error(
                    "Failed to create permission boundary policy. Aborting deployment."
                )
                return False
            ...
            logger.info(
                "Step 3: Deploying IDP stack using service role and permission boundary..."
            )
            ...
            cmd = [
                ...
                "--role-arn",
                service_role_arn,  # Use the service role
                "--parameter-overrides",
            ...
            logger.info(
                f"Successfully deployed stack {self.stack_name} in {self.region}"
            )

            # Step 4: Validate permission boundary on all roles
            logger.info("Step 4: Validating permission boundary on all IAM roles...")
            if not self.validate_permission_boundary(
                self.stack_name, permission_boundary_arn
            ):
