...
def log_progress(stats, schedule):
    while True:
        ...
        print(
            f"[{minutes:02d}:{seconds:02d}] Minute {current_minute}: {current}/{target} files copied (Total: {total})"
        )
        time.sleep(
            5
        )  # semgrep-ignore: arbitrary-sleep - Intentional delay for simulation. Duration is hardcoded and not user-controlled.
...
def read_schedule(csv_file):
    ...
    with open(csv_file, "r", encoding="utf-8-sig") as f:
        ...
        for row in reader:
            if (
                len(row) == 2 and row[0].strip().isdigit()
...
def stress_test(source_bucket, source_key, dest_bucket, dest_prefix, schedule_file):
    ...
    log_thread = threading.Thread(
        target=log_progress, args=(stats, schedule), daemon=True
    )
    log_thread.start()
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        "--schedule-file",
        required=True,
        help="CSV file with minute,copies schedule",
    )
