...
class GovCloudTemplateGenerator:
    ...
    def validate_template_via_s3(self, template_url: str, region: str) -> bool:
        ...
        try:
            self.logger.info(
                f"Performing CloudFormation API validation via S3 URL in region {region}"
            )
            ...
            self.logger.debug(
                f"Template description: {response.get('Description', 'N/A')}"
            )

            # Log any parameters or capabilities
            if response.get("Parameters"):
                self.logger.debug(
                    f"Template has {len(response['Parameters'])} parameters"
                )
            if response.get("Capabilities"):
                self.logger.debug(
                    f"Template requires capabilities: {response['Capabilities']}"
                )
    ...
    def upload_govcloud_template_to_s3(
        self, template_file: str, bucket_name: str, prefix: str, region: str
    ) -> str:
        ...
        try:
            ...
            # Upload the template
            self.logger.info(
                f"Uploading GovCloud template to s3://{bucket_name}/{s3_key}"
            )

            with open(template_file, "rb") as f:
                s3_client.upload_fileobj(
                    f, bucket_name, s3_key, ExtraArgs={"ContentType": "text/yaml"}
                )
    ...
    def update_arn_partitions(self, template: Dict[str, Any]) -> Dict[str, Any]:
        ...
        self.logger.info(
            "Checking ARN partitions (templates should already be partition-aware)"
        )
        ...
        if remaining_arns > 0:
            self.logger.warning(
                f"Found {remaining_arns} hard-coded ARN references that should use partition variable"
            )
    ...
    def clean_cloudfront_policy_statements(
        self, template: Dict[str, Any]
    ) -> Dict[str, Any]:
        """Remove CloudFront-related policy statements from remaining resources"""
        self.logger.info(
            "Removing CloudFront policy statements from remaining resources"
        )
        ...
        for resource_name, resource_def in resources.items():
            ...
            if resource_def.get("Type") == "AWS::S3::BucketPolicy":
                ...
                if self._clean_policy_document(policy_doc, resource_name):
                    self.logger.debug(
                        f"Cleaned CloudFront policy statements from {resource_name}"
                    )
            ...
            # Handle IAM Role policies
                ...
                for policy in policies:
                    if isinstance(policy, dict) and "PolicyDocument" in policy:
                        ...
                        if self._clean_policy_document(
                            policy["PolicyDocument"], f"{resource_name}.{policy_name}"
                        ):
                            self.logger.debug(
                                f"Cleaned CloudFront policy statements from {resource_name}.{policy_name}"
                            )

        return template

    def _clean_policy_document(
            ...
            if isinstance(principal, dict):
                ...
                if isinstance(service, str):
                    ...
                    if "cloudfront." in service.lower():
                        should_remove = True
                        self.logger.debug(
                            f"Removing CloudFront policy statement from {resource_identifier}: {statement.get('Sid', 'unnamed')}"
                        )
                    ...
                    if "Fn::Sub" in service:
                        ...
                        if (
                            ...
                            should_remove = True
                            self.logger.debug(
                                f"Removing CloudFront policy statement (Fn::Sub) from {resource_identifier}: {statement.get('Sid', 'unnamed')}"
                            )
                    ...
                    if len(filtered_services) != len(service):
                        if len(filtered_services) == 0:
                            ...
                            should_remove = True
                            self.logger.debug(
                                f"Removing CloudFront-only policy statement from {resource_identifier}: {statement.get('Sid', 'unnamed')}"
                            )
                        else:
                            ...
                            self.logger.debug(
                                f"Filtered CloudFront services from policy statement in {resource_identifier}: {statement.get('Sid', 'unnamed')}"
                            )
    ...
    def clean_template_for_headless_deployment(
        self, template: Dict[str, Any]
    ) -> Dict[str, Any]:
        ...
        for resource_name, resource_def in resources.items():
            if (
                ...
                and resource_def.get("Type") in s3_bucket_types
            ):
                properties = resource_def.get("Properties", {})
                if "CorsConfiguration" in properties:
                    del properties["CorsConfiguration"]
                    self.logger.debug(
                        f"Removed CORS configuration from {resource_name}"
                    )
        ...
        for func_name in functions_to_convert:
            if func_name in resources:
                ...
                if not dynamodb_policy_exists:
                    ...
                    self.logger.debug(
                        f"Added DynamoDB CRUD permissions for {func_name}"
                    )
                        ...
                        if isinstance(statements, list):
                            ...
                            policy["Statement"] = [
                                ...
                                if not (
                                    ...
                                    and isinstance(stmt.get("Action"), list)
                                    and any(
                                        "appsync:GraphQL" in str(action)
                            ...
                            if len(policy["Statement"]) != len(statements):
                                self.logger.debug(
                                    f"Removed AppSync permissions from {func_name}"
                                )
        ...
        for stack_name in pattern_stacks:
            if stack_name in resources:
                stack_params = (
                    resources[stack_name].get("Properties", {}).get("Parameters", {})
                )
                ...
                if "AppSyncApiUrl" in stack_params:
                    ...
                    self.logger.debug(
                        f"Removed AppSyncApiUrl parameter from {stack_name}"
                    )

                if "AppSyncApiArn" in stack_params:
                    del stack_params["AppSyncApiArn"]
                    self.logger.debug(
                        f"Removed AppSyncApiArn parameter from {stack_name}"
                    )
                ...
                if isinstance(stack_deps, list) and "GraphQLApi" in stack_deps:
                    ...
                    self.logger.debug(
                        f"Removed GraphQLApi dependency from {stack_name}"
                    )
        ...
        if "ShouldUseDocumentKnowledgeBase" in template_str:
            ...
            self.logger.warning(
                "   Note: Knowledge Base functionality disabled for GovCloud compatibility"
            )
        ...
        for output_name, output_def in outputs.items():
            if isinstance(output_def, dict):
                ...
                if (
                    ...
                    and output_value.get("Ref") == "AgentTable"
                ):
                    outputs_to_clean.append(output_name)
                elif isinstance(output_value, dict) and "AgentTable" in str(
                    output_value
                ):
                    outputs_to_clean.append(output_name)
                # Remove outputs that reference WebUIBucket (removed)
                    ...
                    isinstance(output_value, dict)
                    and output_value.get("Ref") == "WebUIBucket"
                ):
                    outputs_to_clean.append(output_name)
        ...
        for group in parameter_groups:
            ...
            if cleaned_params:  # Only keep groups that still have parameters
                ...
                if len(cleaned_params) != len(original_params):
                    self.logger.debug(
                        f"Cleaned {len(original_params) - len(cleaned_params)} params from group: {group_label}"
                    )
            else:
    ...
    def validate_template_basic(self, template: Dict[str, Any]) -> bool:
        ...
        if not present_patterns:
            issues.append(
                "No pattern stacks found - at least one pattern should be present"
            )
    ...
    def print_deployment_summary(
        self, bucket_name: str, prefix: str, region: str, govcloud_url: str = ""
    ):
...
Examples:
    ...
    parser.add_argument(
        "cfn_bucket_basename", help="Base name for the CloudFormation artifacts bucket"
    )
    ...
    parser.add_argument(
        "--max-workers", type=int, help="Maximum number of concurrent workers"
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--skip-build",
        action="store_true",
        help="Skip the build step and only generate/upload GovCloud template",
    )
    parser.add_argument(
        "--clean-build",
        action="store_true",
    ...
    try:
        ...
        if not govcloud_url:
            ...
            if not generator.validate_template_via_s3(govcloud_url, args.region):
                print(
                    "❌ GovCloud template validation failed - template may have issues"
                )
        ...
        generator.print_deployment_summary(
            bucket_name, args.cfn_prefix, args.region, govcloud_url
        )
