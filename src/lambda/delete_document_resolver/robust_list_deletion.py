...
def try_exact_list_deletion(
    ...
    try:
        ...
        if "Attributes" in result:
            logger.info(
                f"Successfully deleted list entry with exact match: {result['Attributes']}"
            )
            return True
        else:
            logger.warning(
...
def query_shard_for_object_key(
    ...
    try:
        logger.info(
            f"Querying shard {list_pk} for entries containing ObjectKey: {object_key}"
        )
        ...
        if "Items" in response:
            for item in response["Items"]:
                ...
                logger.info(
                    f"Found matching list entry: PK={item.get('PK')}, SK={item.get('SK')}"
                )

        # Handle pagination if needed (though filter should make this rare)
        while "LastEvaluatedKey" in response:
            logger.info(
                f"Handling pagination for shard query, continuing from: {response['LastEvaluatedKey']}"
            )
            response = tracking_table.query(
                KeyConditionExpression=Key("PK").eq(list_pk),
                FilterExpression="ObjectKey = :obj_key OR contains(SK, :obj_id)",
            ...
            if "Items" in response:
                for item in response["Items"]:
                    matching_items.append(item)
                    logger.info(
                        f"Found matching list entry (paginated): PK={item.get('PK')}, SK={item.get('SK')}"
                    )
...
def delete_list_entries_robust(
    ...
    if document_metadata:
        ...
        if event_time:
            try:
                ...
                if try_exact_list_deletion(
                    tracking_table, list_pk, list_sk, object_key
                ):
                    deleted_any = True
                    logger.info(
                        "Successfully deleted list entry with exact timestamp match"
                    )
                    return deleted_any  # Success, no need for fallback
                else:
                    logger.warning(
                        "Exact timestamp match failed, proceeding with fallback strategies"
                    )
            except Exception as e:
    ...
    if document_metadata:
        try:
            ...
            if event_time:
                ...
                matching_entries = query_shard_for_object_key(
                ...
                for entry in matching_entries:
                    try:
                        ...
                        if "Attributes" in result:
                            logger.info(
                                f"Successfully deleted list entry via shard query: {entry['SK']}"
                            )
                            deleted_any = True
                    except Exception as e:
                ...
                if not deleted_any:
                    logger.info(
                        "No entries found in calculated shard, checking adjacent shards"
                    )
                    ...
                    for adj_pk in adjacent_shards:
                        ...
                        for entry in matching_entries:
                            try:
                                ...
                                if "Attributes" in result:
                                    logger.info(
                                        f"Successfully deleted list entry via adjacent shard: {entry['SK']}"
                                    )
                                    deleted_any = True
                            except Exception as e:
                                logger.error(
    ...
    if not deleted_any and not document_metadata:
        logger.warning(
            f"No document metadata available for {object_key}, attempting recent date search"
        )
