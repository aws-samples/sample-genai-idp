...
def save_evaluation_to_reporting_bucket(document, reporting_bucket: str) -> None:
    ...
    """
    logger.info(
        f"Writing evaluation results to ReportingBucket s3://{reporting_bucket}/evaluation_metrics/document_metrics"
    )
    try:
        ...
        document_record = {
            ...
            "false_alarm_rate": eval_result.overall_metrics.get(
                "false_alarm_rate", 0.0
            ),
            "false_discovery_rate": eval_result.overall_metrics.get(
        ...
        for section_result in eval_result.section_results:
            ...
            section_record = {
                ...
                "false_discovery_rate": section_result.metrics.get(
                    "false_discovery_rate", 0.0
                ),
                "evaluation_date": now.isoformat(),
            }
            ...
            if has_attributes:
                ...
                for attr in section_result.attributes:
                    attribute_record = {
                        ...
                        "expected_confidence": getattr(
                            attr, "expected_confidence", None
                        ),
                    ...
                    logger.debug(
                        f"Added attribute record for attribute_name={getattr(attr, 'name', '')}"
                    )
        ...
        if section_records:
            ...
            logger.info(
                f"Saved {len(section_records)} section metrics to s3://{reporting_bucket}/{section_key}"
            )
        else:
        ...
        if attribute_records:
            ...
            attribute_lines = "\n".join(
                json.dumps(record) for record in attribute_records
            )
            s3_client.put_object(
                Bucket=reporting_bucket,
                Key=attr_key,
            ...
            logger.info(
                f"Saved {len(attribute_records)} attribute metrics to s3://{reporting_bucket}/{attr_key}"
            )
        else:
