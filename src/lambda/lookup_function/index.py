...
def calculate_durations(timestamps):
    try:
        ...
        if "QueuedTime" in timestamps and "WorkflowStartTime" in timestamps:
            queue_time = (
                datetime.fromisoformat(timestamps["WorkflowStartTime"])
                - datetime.fromisoformat(timestamps["QueuedTime"])
            ).total_seconds() * 1000
            durations["queue"] = int(queue_time)

        if "WorkflowStartTime" in timestamps and "CompletionTime" in timestamps:
            processing_time = (
                datetime.fromisoformat(timestamps["CompletionTime"])
                - datetime.fromisoformat(timestamps["WorkflowStartTime"])
            ).total_seconds() * 1000
        ...
        if "InitialEventTime" in timestamps and "CompletionTime" in timestamps:
            total_time = (
                datetime.fromisoformat(timestamps["CompletionTime"])
                - datetime.fromisoformat(timestamps["InitialEventTime"])
            ).total_seconds() * 1000
...
def get_document_status(object_key, status_only=False):
    ...
    try:
        ...
        response = tracking_table.get_item(
            Key={"PK": PK, "SK": "none"}, ConsistentRead=True
        )
        ...
        if execution_arn:
            try:
                ...
                history = sfn.get_execution_history(
                    executionArn=execution_arn, maxResults=100
                )
                ...
                logger.error(
                    f"Error getting Step Functions details: {e}", exc_info=True
                )
                result["processingDetail"] = {
                    "executionArn": execution_arn,
                    "error": str(e),
        ...
        return {"object_key": object_key, "status": "ERROR", "message": str(e)}
...
def handler(event, context):
    ...
    if object_keys:
        logger.info(
            f"Batch query for {len(object_keys)} documents (status_only={status_only})"
        )
        results = []
