...
def lambda_handler(event, context):
    ...
    try:
        ...
        if not ipset_id and not ipset_name:
            raise ValueError(
                "Either IPSET_ID or IPSET_NAME environment variable must be set"
            )
        ...
        if not lambda_ip_ranges:
            ...
            lambda_ip_ranges = [
                "0.0.0.0/32"
            ]  # Fallback to a dummy IP that won't match anything
        ...
        logger.info(
            f"Successfully updated IPSet with {len(lambda_ip_ranges)} Lambda IP ranges"
        )

        # Send success response for CloudFormation custom resource
        if "ResponseURL" in event:
            response_data["Message"] = (
                f"Updated IPSet with {len(lambda_ip_ranges)} Lambda IP ranges"
            )
            cfnresponse.send(
                event, context, cfnresponse.SUCCESS, response_data, physical_id
            )
    ...
    try:
        ...
        while "NextMarker" in response:
            response = wafv2_client.list_ip_sets(
                Scope="REGIONAL", Limit=100, NextMarker=response["NextMarker"]
            )
        ...
        for ipset in ipsets.get("IPSets", []):
            if ipset["Id"] == ipset_id:
                return wafv2_client.get_ip_set(
                    Name=ipset["Name"], Scope="REGIONAL", Id=ipset_id
                )

        # Handle pagination if needed
        while "NextMarker" in ipsets:
            ipsets = wafv2_client.list_ip_sets(
                Scope="REGIONAL", Limit=100, NextMarker=ipsets["NextMarker"]
            )
