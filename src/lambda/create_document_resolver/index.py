...
def handler(event, context):
    ...
    try:
        ...
        logger.info(
            f"Checking if document {object_key} already exists for user {user_id}"
        )
        existing_doc = None
        try:
            response = tracking_table.get_item(Key={"PK": doc_pk, "SK": doc_sk})
            if "Item" in response:
                existing_doc = response["Item"]
                logger.info(
                    f"Found existing document metadata: {json.dumps(existing_doc, cls=DecimalEncoder)}"
                )
        ...
        if existing_doc:
            try:
                logger.info(
                    f"Attempting robust deletion of list entries for existing document: {object_key}"
                )
                deletion_success = delete_list_entries_robust(
                    tracking_table, object_key, existing_doc
                )

                if deletion_success:
                    logger.info(
                        f"Successfully deleted existing list entries for {object_key}"
                    )
                else:
                    logger.warning(
                        f"No existing list entries found/deleted for {object_key}"
                    )
            except Exception as e:
        ...
        logger.info(
            f"Creating document entries with doc_pk={doc_pk}, list_pk={list_pk}"
        )
        ...
        try:
            ...
            logger.info(
                f"Creating user-scoped document record: PK={doc_pk}, SK={doc_sk}"
            )
