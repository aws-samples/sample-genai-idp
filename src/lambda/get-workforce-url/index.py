        ...
        for attempt in range(max_retries):
            try:
                ...
                if "Workteam" in response and "SubDomain" in response["Workteam"]:
                    ...
                    response_data["ConsoleURL"] = (
                        f"https://{os.environ.get('REGION', 'us-east-1')}.console.aws.amazon.com/sagemaker/groundtruth?region={os.environ.get('REGION', 'us-east-1')}#/labeling-workforces/private"
                    )
                    ...
                    if attempt < max_retries - 1:
                        ...
                        logger.warning(
                            "Maximum retries reached, returning not available"
                        )
            except sagemaker_client.exceptions.ResourceNotFound:
                if attempt < max_retries - 1:
                    logger.info(
                        f"Workteam not found yet. Retrying in {retry_delay} seconds... (Attempt {attempt + 1}/{max_retries})"
                    )
                    time.sleep(retry_delay)
                else:
                    logger.error(
                        f"Workteam {workteam_name} not found after {max_retries} attempts"
                    )
