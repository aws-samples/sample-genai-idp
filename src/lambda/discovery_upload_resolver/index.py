...
def handler(event, context):
    ...
    try:
        ...
        object_key, presigned_post = create_s3_signed_post_url(
            bucket_name, content_type, file_name, "document", prefix
        )
        response = {
        ...
        if ground_truth_file_name:
            gt_object_key, gt_presigned_post = create_s3_signed_post_url(
                bucket_name, content_type, ground_truth_file_name, "groundtruth", prefix
            )
...
def create_discovery_job(job_id, document_key, ground_truth_key):
    ...
    try:
        ...
        if not table_name:
            logger.warning(
                "DISCOVERY_TRACKING_TABLE not configured, skipping job creation"
            )
            return
