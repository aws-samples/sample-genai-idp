...
def batch_copy_s3_objects(source_bucket, destination_bucket, object_keys):
    ...
    s3_client = boto3.client(
        "s3",
        config=Config(
            max_pool_connections=min(len(object_keys), 50), retries={"max_attempts": 3}
        ),
    )
...
def copy_files_async(object_key, source_bucket, destination_bucket):
    ...
    try:
        ...
        operation_parameters = {"Bucket": source_bucket, "Prefix": object_key}
        ...
        if total_objects == 0:
            ...
            if not has_contents:
                logger.warning(
                    f"Prefix {object_key} may not exist in bucket {source_bucket}"
                )
        ...
        logger.info(
            f"Created {len(batches)} batches with average {avg_batch_size:.1f} files per batch"
        )
        ...
        logger.info(
            f"Using {max_workers} workers to process {len(batches)} batches ({total_objects} total files)"
        )
        ...
        with concurrent.futures.ThreadPoolExecutor(max_workers=max_workers) as executor:
            ...
            future_to_batch = {
                executor.submit(
                    batch_copy_s3_objects, source_bucket, destination_bucket, batch
                ): batch
            ...
            for future in concurrent.futures.as_completed(future_to_batch):
                try:
                    ...
                    if total_processed % 100 == 0 or total_processed == total_objects:
                        logger.info(
                            f"Progress: {total_processed}/{total_objects} files processed"
                        )
        ...
        logger.info(
            f"Copied {copied_count}/{total_count} files in {elapsed_time:.2f} seconds "
...
def update_document_copy_status(object_key, evaluation_status=None):
    ...
    try:
        ...
        document = Document(
            id=object_key, input_key=object_key, status=Status.COMPLETED
        )
        ...
        logger.info(
            f"APPSYNC_API_URL from environment: {environ.get('APPSYNC_API_URL')}"
        )
        ...
        logger.info(
            f"Successfully updated document {object_key} with baseline copy status"
        )
        return True
        ...
        logger.error(
            f"Failed to update document with copy status: {str(e)}\nStack trace: {stack_trace}"
        )
        return False
...
def start_async_copy(object_key, source_bucket, destination_bucket):
    ...
    try:
        ...
        if response["StatusCode"] == 202:  # 202 Accepted
            ...
            logger.error(
                f"Failed to start async operation. Status: {response['StatusCode']}"
            )
        ...
        stack_trace = traceback.format_exc()
        logger.error(
            f"Error invoking Lambda asynchronously: {str(e)}\nStack trace: {stack_trace}"
        )
...
def handler(event, context):
    ...
    logger.info(
        f"Lambda context: {context.function_name}, remaining time: {context.get_remaining_time_in_millis()}ms"
    )
    logger.info(
        f"Environment variables: OUTPUT_BUCKET={os.environ.get('OUTPUT_BUCKET')}, EVALUATION_BASELINE_BUCKET={os.environ.get('EVALUATION_BASELINE_BUCKET')}"
    )
    ...
    if "async_operation" in event and event["async_operation"] == "copy_files":
        ...
        logger.info(
            f"Async operation parameters: object_key={object_key}, source_bucket={source_bucket}, destination_bucket={destination_bucket}"
        )
        ...
        try:
            ...
            stack_trace = traceback.format_exc()
            logger.error(
                f"Error in async operation: {str(e)}\nStack trace: {stack_trace}"
            )
            ...
            return {"success": False, "message": f"Error in async operation: {str(e)}"}
    ...
    # Normal GraphQL mutation handling
        ...
        try:
            ...
            logger.info(
                f"Using buckets: source={source_bucket}, destination={destination_bucket}"
            )
            ...
            logger.info(
                f"Will copy all objects under prefix: {source_bucket}/{object_key}/"
            )
