        ...
        for page in sorted_pages:
            if "TextUri" in page:
                ...
                try:
                    ...
                    all_text += (
                        f"<page-number>{page['Id']}</page-number>\n{page_text}\n\n"
                    )
...
def get_summarization_model():
    ...
    try:
        ...
        response = config_table.get_item(Key={"Configuration": "Default"})

        if "Item" in response:
            config_data = response["Item"]
            # Extract summarization model from the configuration
            if (
                "summarization" in config_data
                and "model" in config_data["summarization"]
            ):
...
def handler(event, context):
    ...
    try:
        ...
        if len(objectKey):
            ...
            logger.info(f"New response: {cleaned_up_text}")
