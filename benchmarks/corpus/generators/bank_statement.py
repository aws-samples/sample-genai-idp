...
MERCHANTS = ["AnyCompany Store", "Example Mart", "Sample Cafe", "Test Fuel Co",
             "Demo Pharmacy", "Placeholder Books", "Acme Utilities", "Widget Depot"]
WIDE_COLS = ["Date", "Description", "Amount", "Balance", "Category", "Ref", "Type", "Location"]
...
def _row(seq, cols, desc_len, noise):
    ...
    if desc_len == "long":
        desc = (f"SEQ{seq:05d} Purchase at {merch} - authorization code "
    ...
    return [date, desc, amount, f"{(seq * 31.1) % 99999:.2f}",
            ["Food", "Fuel", "Retail", "Bills"][seq % 4], f"R{seq:06d}",
            ["DEBIT", "CREDIT"][seq % 2], f"City{seq % 40}, ST"]
...
def build(rows, cols=3, lists=1, desc_len="short", ocr_noise=0.0, out="doc.pdf"):
    ...
    doc = SimpleDocTemplate(out, pagesize=letter, topMargin=0.5 * inch,
                            bottomMargin=0.5 * inch, leftMargin=0.4 * inch, rightMargin=0.4 * inch)
    story = [Paragraph("<b>AnyBank Monthly Statement</b>", styles["Title"]),
             ...
             Paragraph("Account Holder: Jane Q Public, 100 Main Street, Anytown, CA 90210", styles["Normal"]),
             Spacer(1, 0.2 * inch)]
    ...
    for li in range(lists):
        ...
        if lists > 1:
            story.append(Paragraph(f"<b>Transaction Ledger {li + 1}</b>", styles["Heading2"]))
        data = [header]
        ids = []
        for _ in range(n):
    ...
    doc.build(story)
    truth = {"gen": "bank_statement", "rows": rows, "cols": cols, "lists": lists,
             "desc_len": desc_len, "ocr_noise": ocr_noise,
             "fields": dict(FIELDS), "seq_ids": seq_ids,
