...
sys.path.insert(
    0, os.path.join(os.path.dirname(__file__), "../../../../src/lambda/upload_resolver")
)
...
class TestHandlerLogging:
    ...
    def test_logs_user_extraction(
        ...
        assert (
            f"Processing upload request for user: {valid_cognito_uuid}" in caplog.text
        )
    ...
    def test_logs_user_scoped_path(
        ...
        # Verify path logging
        assert (
            f"User-scoped upload path: users/{valid_cognito_uuid}/invoice-2025.pdf"
            in caplog.text
        )
