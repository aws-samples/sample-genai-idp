...
class TestHandlerDocumentCreation:
    ...
    def test_creates_list_item_with_shard(
        ...
        assert (
            list_item["UserId"] == valid_cognito_uuid
        )  # CRITICAL: UserId must be in list item

    @patch("index.dynamodb")
    def test_uses_sub_when_username_missing(
        self,
        mock_dynamodb,
        create_document_event_with_sub,
        mock_lambda_context,
        valid_cognito_uuid,
    ):
...
class TestHandlerExistingDocuments:
    ...
    def test_continues_on_get_item_failure(
        self, mock_dynamodb, valid_create_document_event, mock_lambda_context, caplog
    ):
...
class TestHandlerValidation:
    ...
    @patch("index.dynamodb")
    def test_raises_error_on_missing_object_key(
        self, mock_dynamodb, valid_cognito_uuid
    ):
    ...
    @patch("index.dynamodb")
    def test_raises_error_on_missing_queued_time(
        self, mock_dynamodb, valid_cognito_uuid
    ):
