...
sys.path.insert(
    0, os.path.join(os.path.dirname(__file__), "../../../../src/lambda/queue_sender")
)
...
class TestHandlerMessageSending:
    ...
    def test_sends_message_with_user_context(
        ...
        assert (
            message_body["ObjectKey"] == f"users/{valid_cognito_uuid}/test-document.pdf"
        )
        ...
        assert (
            attrs["ObjectKey"]["StringValue"]
            == f"users/{valid_cognito_uuid}/test-document.pdf"
        )
    ...
    def test_handles_nested_user_paths(
        ...
        assert message_body["UserId"] == valid_cognito_uuid
        assert (
            message_body["ObjectKey"]
            == f"users/{valid_cognito_uuid}/subfolder/nested/document.pdf"
        )
...
class TestHandlerValidation:
    ...
    def test_raises_error_on_missing_bucket_name(self, mock_sqs, mock_lambda_context):
        ...
        event = {"detail": {"object": {"key": "users/test-user/doc.pdf"}}}
...
class TestHandlerLogging:
    ...
    def test_logs_event_details(
        self, mock_sqs, valid_s3_event, mock_lambda_context, mock_sqs_response, caplog
    ):
    ...
    def test_logs_extracted_user_id(
        ...
        assert (
            f"Sending message to SQS with UserId: {valid_cognito_uuid}" in caplog.text
        )
